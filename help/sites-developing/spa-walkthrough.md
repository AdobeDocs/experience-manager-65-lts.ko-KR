...
* [GitHub에서 사용 가능한 샘플 WKND SPA Project 앱](https://github.com/adobe/aem-guides-wknd-spa)
   * React 앱의 [최신 릴리스를 다운로드합니다.](https://github.com/adobe/aem-guides-wknd-spa/releases) `wknd-spa-react.all.classic-X.Y.Z-SNAPSHOT.zip`과(와) 유사한 이름이 지정됩니다.
   * 앱에 대한 [최신 샘플 이미지](https://github.com/adobe/aem-guides-wknd-spa/releases)를 다운로드합니다. `wknd-spa-sample-images-X.Y.Z.zip`과(와) 유사한 이름이 지정됩니다.
...
>[!CAUTION]
>
>이 문서에서는 데모용으로만 [WKND Spa 프로젝트 앱](https://github.com/adobe/aem-guides-wknd-spa)을 사용합니다. 프로젝트 작업에는 사용하지 마십시오.
>
>모든 AEM 프로젝트에서는 React 또는 Angular을 사용하는 SPA 프로젝트를 지원하고 SPA SDK를 사용하는 [AEM 프로젝트 Archetype](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=ko)을 사용해야 합니다.
...
1. 머리글 구성 요소를 선택하고 도구 모음이 다른 구성 요소에서와 같이 표시되는지 확인합니다. **편집**&#x200B;을 선택합니다.

   ![단계 2](assets/spa-walkthrough-step-2.png)

1. AEM 내에서 내용을 정상적으로 편집합니다. 변경 내용은 그대로 유지됩니다.
...
1. 자산 브라우저를 사용하여 새 이미지를 이미지 구성 요소로 드래그 앤 드롭합니다.
   ...
   ![4](assets/spa-walkthrough-step-4.png) 단계

1. 변경 사항이 지속됩니다.

   ![단계 5](assets/spa-walkthrough-step-5.png)

페이지에서 추가 구성 요소를 드래그하여 놓기, 구성 요소 재정렬, 레이아웃 수정 등의 추가 작성 도구는 비 SPA 응용 프로그램에서와 마찬가지로 지원됩니다.
...
1. 브라우저 기본 제공 개발자 도구를 열고 하위 페이지를 탐색하면서 네트워크 활동을 모니터링합니다.
   ...
   ![단계 3](assets/spa-walkthrough-step-1-3.png)
...
다음 섹션 [SPA 응용 프로그램 불러오기](#loading-an-spa-application)에서는 SPA을 불러오는 방법과 콘텐츠를 동기적 및 비동기적으로 불러오는 방법에 대해 자세히 살펴봅니다.
...
1. 아직 로드되지 않은 경우에는 게시 서버에서 또는 페이지 편집기의 **페이지 정보** 메뉴에서 **게시됨으로 보기** 옵션을 사용하여 WKND SPA 프로젝트 앱을 편집기에 로드합니다.
   ...
   ![단계 1](assets/spa-walkthrough-step-1-1.png)
...
1. 개발자 도구의 **네트워크** 탭으로 전환하고 페이지를 다시 로드합니다.
   ...
   ![단계 5](assets/spa-walkthrough-step-1-5.png)
...
1. 새 탭에 `react.model.json`을 로드합니다.
   ...
   각 구성 요소는 노출 항목과 렌더링 방법(HTL이 있는 서버측 또는 React가 있는 클라이언트측)을 선택합니다. 이 문서는 React를 통해 클라이언트측 렌더링에 중점을 둡니다.
...
1. 브라우저의 기본 제공 개발자 도구를 사용하여 페이지 콘텐츠를 검사합니다. 선택 도구를 사용하여 페이지에서 편집 가능한 구성 요소를 선택하고 요소 세부 정보를 조회합니다.
   ...
   ![단계 2](assets/spa-walkthrough-step-2-2.png)
   ...
   >[!NOTE]
   >
   >이는 각 편집 가능한 구성 요소에 대해 삽입된 `cq` 요소가 있는 AEM의 서버측 렌더링 페이지에서 변경된 동작입니다.
   >
   >
   >SPA의 이러한 접근 방식은 추가 데이터 특성만 사용하고 사용자 정의 요소를 주입할 필요가 없기 때문에 프런트 엔드 개발자가 마크업을 더 간단하게 만들 수 있습니다.
