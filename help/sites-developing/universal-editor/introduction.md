---
title: 범용 편집기
description: 유니버설 편집기의 유연성과 AEM 6.5 LTS를 사용하여 헤드리스 경험을 강화하도록 지원하는 방법에 대해 알아보십시오.
feature: Developing
role: Developer
...
유니버설 편집기의 유연성과 AEM 6.5 LTS를 사용하여 헤드리스 경험을 강화하도록 지원하는 방법에 대해 알아보십시오.

## 개요 {#overview}

범용 편집기는 Adobe Experience Manager Sites의 일부인 다용도 시각적 편집기입니다. 이를 통해 작성자는 Headless 경험에 대한 보이는 것(WYSIWYG) 편집을 수행할 수 있습니다.

* 작성자는 Universal Editor의 유연성을 활용할 수 있습니다. 모든 형태의 AEM 헤드리스 콘텐츠에 대해 동일한 일관된 시각적 편집을 지원합니다.
* 개발자는 범용 편집기의 다목적성을 활용하여 구현을 완전히 분리할 수 있습니다. 이를 통해 개발자는 SDK나 기술 제약 없이 원하는 프레임워크나 아키텍처를 거의 사용할 수 있습니다.
...
이 문서에서는 AEM 6.5 LTS의 유니버설 편집기 지원에 대해 중점적으로 설명합니다. AEM 6.5 LTS에서 범용 편집기를 사용하려면 다음이 필요합니다.
...
유니버설 편집기를 사용하려면 다음을 수행하십시오.

1. [AEM 제작 인스턴스에서 서비스를 구성합니다.](#configure-aem)
...
#### Adobe Granite Query Parameter Authentication Handler 구성 {#query-parameter}
...
1. `Universal Editor Opening Mapping`에 정의된 경로 밖의 콘텐츠에 대해 AEM은 `resourceType` 콘텐츠가 **Sling:resourceTypes에서 유니버설 편집기가 열 항목과 일치하는지 확인합니다**. 일치하는 경우 AEM은 `${author}${path}.html`의 유니버설 편집기에서 콘텐츠를 엽니다.
1. 그렇지 않으면 AEM에서 [페이지 편집기]가 열립니다.
...
* `author`: 스키마가 없는 작성자에 대한 Externalizer 항목(예: `localhost:4502`)
* `publish`: 스키마 없이 게시하기 위한 Externalizer 항목(예: `localhost:4503`)
* `preview`: 스키마 없는 미리 보기에 대한 Externalizer 항목입니다(예: `localhost:4504`).
...
AEM이 구성되고 로컬 유니버설 편집기 서비스가 실행 중인 경우 Dispatcher에서 새 서비스 [에 대한 역방향 프록시를 허용해야 합니다.](https://experienceleague.adobe.com/ko/docs/experience-manager-dispatcher/using/dispatcher)
...
AEM 6.5 LTS의 Universal Editor는 UI 및 대부분의 설정을 포함하여 AEM에서 Cloud Service와 동일하게 작동합니다. 그러나 유의해야 할 차이점이 있습니다.

* 6.5 LTS의 Universal Editor는 헤드리스 사용 사례만 지원합니다.
* 유니버설 편집기의 설정은 6.5 LTS에 대해 약간 다릅니다([현재 문서에 설명된 대로](#setup)).
