...
* 맞춤화된 정의의 내용은 원본보다 우선 순위가 높습니다. 즉, *오버레이* 또는 *재정의*&#x200B;됩니다.
...
* `/libs`에서 복제되는 구조를 줄이십시오.
  ...
  Sling 리소스 병합을 사용하는 경우 `/libs`에서 전체 구조를 복사하지 않는 것이 좋습니다. 그 이유는 맞춤화에 너무 많은 정보가 포함될 수 있기 때문입니다(일반적으로 `/apps`). 정보가 중복되면 시스템 업그레이드 시 문제가 발생할 가능성이 불필요하게 증가한다.

>[!NOTE]
>
>재정의는 검색 경로에 종속되지 않습니다. `sling:resourceSuperType` 속성을 사용하여 연결합니다.
>
>그러나 AEM의 모범 사례는 `/apps`에서 사용자 지정을 정의하는 것이므로 `/apps`에서 재정의가 정의되는 경우가 많습니다. 그 이유는 `/libs`에서 변경할 수 없기 때문입니다.
...
* `sling:hideChildren`( `String` 또는 `String[]`)
  ...
  숨길 하위 노드 또는 하위 노드 목록이 포함되어 있습니다. 노드의 속성은 유지됩니다.
...
이러한 속성은 해당/원래 리소스/속성(`/libs`부터)을 오버레이/재정의(종종 `/apps`에서)에서 사용하는 방식에 영향을 줍니다.
...
>[!NOTE]
>
>Sling 리소스 병합을 사용하는 경우(즉, 표준 터치 사용 UI를 처리하는 경우) `/libs`에서 전체 구조를 복사하지 않는 것이 좋습니다. 그 이유는 `/apps`에 너무 많은 정보가 보관되기 때문입니다. 결과적으로 시스템 업그레이드 시 문제가 발생할 수 있습니다.

### 사용 사례 {#use-cases}

표준 기능을 사용하면 이러한 사용 사례에서 다음을 수행할 수 있습니다.
...
Sling 리소스 병합에는 두 개의 사용자 지정 리소스 공급자가 포함되어 있습니다. 하나는 오버레이에 사용되고 다른 하나는 재정의에 사용됩니다. 각각은 마운트 지점을 사용하여 코드 내에서 호출할 수 있습니다.

>[!NOTE]
>
>리소스에 액세스할 때는 적절한 마운트 지점을 사용하는 것이 좋습니다.
>
>이 방법은 Sling 리소스 병합을 호출하고 완전히 병합된 리소스를 반환합니다. `/libs`에서 복사해야 하는 구조의 양도 줄어듭니다.
