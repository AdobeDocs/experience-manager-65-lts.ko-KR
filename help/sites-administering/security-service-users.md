...
>[!IMPORTANT]
>
>CRX 탐색기(`/crx/explorer/index.jsp`)는 AEM 6.5 LTS 환경에서 사용할 수 없으므로 서비스 사용자를 만드는 데 사용해서는 안 됩니다. CRX 탐색기를 통해 만든 기존 서비스 사용자는 계속 작동합니다. 신규 서비스 사용자의 경우 아래 설명된 접근 방식 중 하나를 사용하십시오.

>[!NOTE]
>
>JCR 노드 수준에서 서비스 사용자와 연결된 mixin 유형이 없습니다. 즉, 시스템 사용자 노드에는 액세스 제어 정책이 직접 첨부되어 있지 않습니다. RepoInit ACL 문이나 저장소 수준 ACL 구성 등을 통해 액세스 제어를 별도로 관리합니다.

### Sling 저장소 초기화 사용(RepoInit) {#creating-service-user-repoinit}
...
`with path system/cq` 지시문은 저장소의 `/home/users/system/cq` 아래에 서비스 사용자를 배치합니다. 프로젝트의 조직 구조와 일치하는 경로를 선택할 수 있습니다(예: `system/myproject`). 중간 경로 노드가 없으면 `with forced path`을(를) 사용하여 자동으로 만드십시오.

다음과 같은 이유로 이 방법을 사용하는 것이 좋습니다.

* 서비스 사용자 및 권한을 코드로 정의하여 버전을 제어하고 재현 가능
* 저장소 초기화 중 생성을 자동으로 처리합니다.
* AEM 6.5 LTS 및 AEM as a Cloud Service 환경 모두에서 작동하지만 Sling 버전 간에는 구문 차이가 적을 수 있습니다. 대상 플랫폼에 대한 RepoInit 설명서를 참조하십시오

### 컨텐츠 패키지 사용 {#creating-service-user-content-package}

콘텐츠 패키지에 `.content.xml`을(를) 포함하여 서비스 사용자를 만들 수도 있습니다. `rep:authorizableId`을(를) 설정했으며 기본 유형이 `rep:SystemUser`인지 확인하십시오. 콘텐츠 패키지를 설치하는 동안 사용자를 올바르게 만들려면 유효한 `jcr:uuid` 속성이 필요합니다. 표준 UUID v4 생성기(예: `uuidgen` 명령줄 도구 또는 모든 온라인 UUID 생성기)를 사용하여 UUID를 생성할 수 있습니다. `.content.xml`은(는) 다음과 같아야 합니다.
