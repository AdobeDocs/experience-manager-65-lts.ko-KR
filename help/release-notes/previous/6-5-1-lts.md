...
>[!IMPORTANT]
>
> **필수 핫픽스** – 서비스 팩 1을 설치할 때 오프라인 압축과 관련된 SNFE(SegmentNotFoundException) 문제를 방지하려면 [알려진 문제 - 온라인 압축 중 저장소 손상](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146)에 설명된 핫픽스를 설치하십시오.
...
#### AEM Forms 6.5 LTS SP1 포함 사항

**Java 지원 업데이트**

최신 Java 버전에 대한 지원이 도입되었습니다.

* Oracle Java™ 17/21(64비트)(자세한 내용은 [Java 지원](#java-support) 섹션 참조.

**애플리케이션 서버 지원 업데이트**

* JBoss® EAP 8에 대한 지원이 추가되었습니다.
* 이전 PicketBox 보안 프레임워크가 제거되었습니다.
* 이제 보안 자격 증명 관리를 위해 Elytron 기반 자격 증명 저장소가 지원됩니다.

**구성: 자격 증명 저장소(Elytron 기반)**

JBoss® EAP 8의 AEM Forms이 보안 자격 증명을 관리하는 데 Elytron을 사용합니다. 고객은 서버를 성공적으로 시작하고 데이터베이스 인증을 보호하려면 Elytron 기반 자격 증명 스토어를 구성해야 합니다.
...
* 서블릿 사양 5+ 지원
* Jakarta EE 9 규정 준수를 기반으로 함

**네임스페이스 마이그레이션 요구 사항**

* Jakarta EE 9에서 네임스페이스가 `javax.*`에서 `jakarta.*`로 변경되었습니다.
* 모든 **사용자 정의 DSC**&#x200B;를 `jakarta.*` 네임스페이스로 마이그레이션해야 합니다.
* AEM Forms 6.5 LTS SP1은 **Jakarta EE 9+ 기반 애플리케이션 서버만** 지원

자세한 내용은 **Javax에서 Jakarta 네임스페이스로 마이그레이션**&#x200B;을 참조하십시오.

#### `javax`에서 `jakarta` 네임스페이스로 마이그레이션

**AEM Forms 6.5 LTS SP1**&#x200B;부터 **Jakarta Servlet API 5/6**&#x200B;를 구현하는 애플리케이션 서버만 지원됩니다. **Jakarta EE 9 이상**&#x200B;에서 모든 API가 `javax.{}` 네임스페이스에서 `jakarta.`로 전환되었습니다.

따라서 **모든 사용자 정의 DSC는 `jakarta` 네임스페이스를 사용해야 합니다**. `javax.{}` API를 사용하여 빌드된 사용자 정의 구성 요소는 지원되는 애플리케이션 서버와 **호환되지 않습니다**.

**사용자 정의 DSC에 대한 마이그레이션 옵션**

다음 방법 중 하나를 사용하여 기존 사용자 정의 DSC를 마이그레이션할 수 있습니다.

**옵션 1: 소스 코드 마이그레이션(권장)**

* `javax.{}`에서 `jakarta.`로 모든 가져오기 구문 업데이트
* 사용자 정의 DSC 프로젝트 다시 빌드 및 다시 컴파일
...
* **Eclipse 변환기** 도구를 사용하여 컴파일된 바이너리 파일(`.jar`, `.war`)을 `javax`에서 `jakarta`로 변환합니다
* 소스 코드를 변경하거나 다시 컴파일할 필요가 없습니다.
* 변환된 바이너리를 애플리케이션 서버에 다시 배포

>[!NOTE]
>
> 바이너리 변환은 **바이트코드 수준**&#x200B;에서 수행됩니다.
...
다음 테이블은 `javax`에서 `jakarta`로 마이그레이션할 때 필요한 일반적인 네임스페이스 변경 사항을 보여 줍니다.
...
사용자 정의 DSC 소스 코드를 업데이트하거나 변환된 바이너리를 확인할 때 이러한 매핑을 참조로 사용합니다.
...
* 폴더를 만들 때 확인 메시지가 표시되지 않고 JAWS가 에셋 UI에서 이를 알리지 않는 접근성 문제를 해결했습니다. (SITES-27141)
...
* NVDA가 이제 트리 디렉터리에 수정 날짜를 알려 화면 판독기 사용자가 완전한 에셋 세부 정보를 받을 수 있도록 했습니다. (SITES-24782)
...
* 사이트 관리자 UI에서 올바른 포커스 탐색을 통해 논리적 순서를 따르도록 수정했습니다. 따라서 Esc 키를 누르면 포커스가 &#39;모두 선택&#39; 버튼으로 돌아가고, 탭 키를 누르면 포커스가 다음 대화형 요소로 이동합니다. (SITES-24307)
...
이제 AEM이 이미지 에셋의 잘못된 XMP 메타데이터로 인해 발생하는 성능 저하를 방지합니다. 숫자 세그먼트나 정규화되지 않은 구조 등 유효하지 않거나 규정을 준수하지 않는 XMP 속성 이름이 포함된 에셋은 처리 중에 더 이상 반복적인 경고 로그를 트리거하지 않습니다. 이 시스템은 문제가 있는 메타데이터를 필터링하여 에셋 수집 및 검증이 오류 없이 완료되도록 보장합니다. (SITES-30683)
...
작성자가 콘텐츠 조각을 체크아웃한 경우에도 다른 작성자가 여전히 콘텐츠 조각을 게시할 수 있다면 체크아웃 기능의 의도된 동작에 반하게 됩니다. 이번 수정 사항은 콘텐츠 조각이 체크아웃될 때 다른 사용자가 저작 인터페이스에서 게시 버튼을 보거나 사용할 수 없도록 합니다. (SITES-30578)
...
XMP 메타데이터가 잘못 포맷되어 `ValidationDataServlet`에서 이미지 에셋을 처리하는 동안 오류가 발생하는 문제가 있었습니다. 이번 수정 사항은 규정을 준수하는 메타데이터의 처리를 보장하고 잘못된 속성의 중복 구문 분석을 방지합니다. (SITE-30683)
...
원격 DAM에서 Sites 로컬 AEM으로 에셋을 동기화할 때 에셋에서 게시된 상태 및 복제 관련 속성이 제거되는 문제를 해결했습니다. (Assets-48958)
...
* 론치 구성의 속성 섹션에서 회사 및 속성 드롭다운이 나타나지 않는 문제를 해결했습니다. 또한 모든 필수 필드가 채워졌음에도 불구하고 저장 및 닫기에서 발생하는 오류와 제목 필드만 비어 있을 때 회사 및 부동산에 대한 잘못된 오류 메시지가 표시되는 문제를 해결했습니다. (CQ-4359853)
...
* Firefox에서 프로젝트 보기의 &#39;에셋 컬렉션&#39; 카드 바닥글이 카드 테두리와 겹치는 문제를 해결했습니다. 이제 바닥글이 겹치지 않고 올바르게 정렬됩니다. (CQ-4353317)
...
* AEM 6.5를 위한 유지 관리 및 지속 전략을 수립했습니다. 이 수정 사항에는 다음 내용이 포함됩니다.
   * 서비스 팩 주기
   * 핫픽스 주기
   * AEM 6.6과의 병렬 지원
   * 지원 매트릭스 업데이트
...
>[!NOTE]
>
> (Forms만 해당) Oracle Java™ 8/11(64비트)은 일회성 설치에 필요합니다.
...
**이것이 중요한 이유**
AEM 6.5 LTS용 SP1이 패키지 관리자를 통해 설치하기 위해 ZIP이 아닌 Quickstart JAR로 제공됩니다. On-prem 고객은 Quickstart JAR를 대체하고 압축을 푼 다음 다시 시작하여 업그레이드합니다. 이 방법은 Adobe의 즉석 업그레이드 절차와 일치합니다.
...
1. 실행 중인 인스턴스를 중단합니다.
1. AEM 설치 디렉터리(`crx-quickstart/` 외부)에서 이전 Quickstart JAR를 SP1 JAR로 대체합니다.
1. JAR 압축을 풉니다.
...
* **FORMS-24688:** Linux에서 JBoss EAP 8.0(AEM Forms 6.5.1 LTS)을 설정할 때 Windows에서 만들거나 편집한 스크립트에 CRLF 줄 끝이 포함되어 있어 /bin/sh^M: 잘못된 인터프리터 또는 $&#39;\r&#39;: 명령에 오류가 없습니다. 영향을 받는 파일이 LF 줄 끝을 사용하는지 확인합니다. 이는 dos2unix와 같은 도구나 이와 동등한 방법을 사용하여 수행할 수 있습니다.

* **FORMS-24689:** 턴키 모드에서 새로 설치하거나 업그레이드하는 동안 데이터베이스 포트 업데이트가 제대로 적용되지 않습니다. 새로 설치 모드에서는 화면에 표시된 포트만 선택할 수 있습니다. 업그레이드 모드에서는 업그레이드를 진행하기 전에 lc_turnkey.xml에서 새 데이터베이스 포트를 수동으로 업데이트해야 합니다.

* **FORMS-24687:** JBoss EAP 8에 대한 AEM Forms 6.5 LTS 클러스터 배포에서 domain/configuration/domain_oracle.xml, domain_mysql.xml 및 domain_mssql.xml 파일에 중복 `<security>` 태그가 포함되어 있어 도메인 컨트롤러가 시작되지 않도록 하는 잘못된 XML이 생성될 수 있습니다.
...
이전에 JCR 저장소에서 오프라인 압축이 실행된 경우 온라인 압축 중에 저장소 손상이 발생했습니다. 이 시나리오에서는 `SegmentNotFoundException`(SNFE)이 발생할 수 있습니다. 이로 인해 저장소가 손상될 수 있습니다.

이 문제를 해결하려면 [소프트웨어 배포](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip)에서 패키지 관리자를 통해 핫픽스를 설치합니다. 핫픽스에 낮은 수준의 `oak-segment-tar` 번들이 포함되어 있으므로 설치 후 인스턴스가 다시 시작됩니다.

인스턴스를 적용할 때 인스턴스의 다운타임에 대한 계획을 수립합니다. 오프라인 압축을 위해 소프트웨어 배포에서도 사용할 수 있는 해당 [Oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar)를 사용합니다.

>[!NOTE]
>
> Oak-run 작업의 경우 [Oak-run 1.78.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar)&#x200B;(위에 연결됨)를 사용합니다.
...
### AEM 6.5 LTS SP1에 `com.adobe.granite.apicontroller` 번들이 없습니다(GRANITE-67639). {#missing-apicontroller-bundle-granite-67639}

AEM 6.5 LTS SP1에 `com.adobe.granite.apicontroller` 번들이 없습니다. 이 번들은 OSGi 번들이 확인되는 방법을 제어하며 번들이 다른 번들로 확인되지 않도록 할 수 있습니다. 이는 노출된 API를 제한하는 데 유용합니다.

이 기능을 사용하려면 [소프트웨어 배포](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-67639-1.0.zip)에서 핫픽스를 설치하십시오.
...
* [Experience Manager 6.5 LTS, 서비스 팩 1에 포함된 OSGi 번들 목록](/help/release-notes/assets/65lts_sp1_bundles.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager 6.5 LTS, 서비스 팩 1에 포함된 콘텐츠 패키지 목록](/help/release-notes/assets/65lts_sp1_packages.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
