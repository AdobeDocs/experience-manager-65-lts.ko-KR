...
* Oracle Java™ 17/21(64비트)([Java 지원](#java-support) 섹션에서 자세한 내용을 참조하십시오.
...
자세한 내용은 **Javax에서 Jakarta 네임스페이스로 마이그레이션**&#x200B;을 참조하세요.
...
>[!NOTE]
>
> (Forms만 해당) Oracle Java™ 8/11(64비트)은 1회 설치에 필요합니다.
...
**이 문제가 되는 이유**
AEM 6.5용 SP1 LTS는 패키지 관리자를 통해 설치하기 위해 ZIP이 아닌 Quickstart JAR로 제공됩니다. On-prem 고객은 Quickstart JAR을 대체하고 압축을 푼 다음 다시 시작하여 업그레이드합니다. 이 방법은 Adobe의 즉석 업그레이드 절차와 일치합니다.
...
* **FORMS-24688:** Linux에서 JBoss EAP 8.0(AEM Forms 6.5.1 LTS)을 설정할 때 Windows에서 만들거나 편집한 스크립트에 CRLF 줄 끝이 포함되어 있어 /bin/sh^M: 잘못된 인터프리터 또는 $&#39;\r&#39;: 명령에 오류가 없습니다. 영향을 받는 파일이 LF 줄 끝을 사용하는지 확인합니다. 이 작업은 dos2unix와 같은 도구나 이와 동등한 방법을 사용하여 수행할 수 있습니다.
...
오프라인 압축을 이전에 JCR 저장소에서 실행한 경우 온라인 압축 중에 저장소 손상이 발생했습니다. 이 시나리오에서는 `SegmentNotFoundException`(SNFE)이 발생할 수 있습니다. 이로 인해 저장소가 손상될 수 있습니다.

이 문제를 해결하려면 [소프트웨어 배포](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip)에서 패키지 관리자를 통해 핫픽스를 설치하십시오. 핫픽스에 낮은 수준의 `oak-segment-tar` 번들이 포함되어 있으므로 설치 후 인스턴스가 다시 시작됩니다.

인스턴스를 적용할 때 인스턴스의 다운타임에 대한 계획을 수립합니다. 오프라인 압축을 위해 해당 [Oak-run jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar)을(를) 사용합니다. 소프트웨어 배포에서도 사용할 수 있습니다.

>[!NOTE]
>
> Oak 실행 작업의 경우 [Oak-run 1.78.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar)&#x200B;(위에 연결됨)을 사용하십시오.
