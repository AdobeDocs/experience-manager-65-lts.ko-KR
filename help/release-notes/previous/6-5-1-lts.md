...
* **FORMS-24688:** Linux에서 JBoss EAP 8.0(AEM Forms 6.5.1 LTS)을 설정할 때 Windows에서 수정된 스크립트에 CRLF 줄 끝이 포함되어 있어 /bin/sh^M: 잘못된 인터프리터 또는 $&#39;\r&#39;: 명령이 오류를 찾을 수 없습니다. dos2unix를 사용하여 영향을 받는 .sh 파일을 Unix(LF) 형식으로 변환합니다(예: 찾기 ). -type f -name &quot;*.sh&quot; -exec dos2unix {} +) 문제를 해결합니다.

* **FORMS-24689:** 턴키 업그레이드 모드에서는 데이터베이스 포트 업데이트가 UI에서 선택되지 않으며 lc_turnkey.xml 구성 파일에서 수동으로 수정해야 합니다. 올바르게 업데이트되지 않으면 시스템에서 이전 포트 값을 계속 사용할 수 있습니다.

* **FORMS-24687:** JBoss EAP 8에 대한 AEM Forms 6.5.1 LTS 클러스터 배포에서 `domain/configuration/domain_oracle.xml`에 중복된 `<security>` 태그가 포함되어 있어서 도메인 컨트롤러가 시작되지 않는 잘못된 XML이 발생할 수 있습니다.
