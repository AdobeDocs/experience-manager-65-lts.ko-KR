...
* **FORMS-24688:** Linux에서 JBoss EAP 8.0을 설정할 때 Windows에서 수정된 셸 스크립트에 CRLF 줄 끝이 포함되어 있어 `/bin/sh^M: bad interpreter` 또는 $`\r`: 명령이 오류를 찾을 수 없습니다. 이 문제를 해결하기 위해 dos2unix를 사용하여 영향을 받는 파일을 LF 줄 끝으로 변환합니다.

* **FORMS-24689:** 턴키 업그레이드 모드에서 `lc_turnkey.xml`의 데이터베이스 포트 업데이트가 업그레이드 중에 제대로 적용되지 않아 이전 포트 값을 참조할 수 있습니다.
