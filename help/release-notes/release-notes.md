...
* FORMS-24687: JBoss EAP 8에 대한 AEM Forms 6.5 LTS 클러스터 배포에서 `domain/configuration/domain_oracle.xml`에 잘못된 XML을 발생시키고 도메인 컨트롤러를 시작하지 못하게 하는 중복 `<security>` 태그가 더 이상 없습니다.

* FORMS-24689:In 턴키 업그레이드 모드에서 `lc_turnkey.xml`의 데이터베이스 포트 업데이트가 업그레이드 중에 올바르게 적용되며 더 이상 이전 포트 값을 참조하지 않습니다.
...
* **FORMS-24690:** 모듈을 선택하지 않았거나 제한된 구성 요소만 선택한 경우 Configuration Manager에서 AEM Forms 6.5 LTS JEE 턴키 사용자 지정 모드의 부트스트랩 중에 데이터베이스 초기화가 실패할 수 있습니다. 이 오류는 종속성 누락(xalan-2.7.2.jar)으로 인해 오류가 발생했기 때문에 발생합니다. adobe-livecycle-jboss.ear\lib에 JAR 파일을 추가하면 문제가 해결됩니다.
