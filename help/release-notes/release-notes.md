...
|---|---|
| 버전 | 서비스 팩 1(SP1), GRANITE-61551 <!-- UPDATE FOR EACH NEW RELEASE -->용 핫픽스 |
| 유형 | 서비스 팩 릴리스 |
...
* 패키지 설치 중에 `DataTimeParseException` 및 `String.length()` null 포인터 예외를 유발하는 Sling 스크립팅 문제에 대한 수정 사항을 백포트했습니다. 설치 오류를 줄이고 안정성을 개선하기 위해 Sling Scripting을 버전 2.8.3-1.0.10.6으로 업데이트했습니다. (NPR-42640)
...
* 출력 서비스를 사용하여 동적이고 입력 가능한 PDF를 PDF/A 형식으로 평면화할 때 동적 상태가 보존되지 않습니다. 이로 인해 데이터가 손실되고, 특히 태그 지정이 활성화된 경우 잠재적인 규정 준수 문제가 발생합니다. (LC-3922708)
...
* 사용자는 AEM Forms Designer 또는 출력 서비스를 사용하여 XDP 양식을 정적 PDF로 변환할 때 단일 통합 링크 태그를 생성하는 대신 여러 Link-OBJR 태그가 생성되는 접근성 규정 준수 문제에 직면합니다. (LC-3921977)
...
* 시스템이 JSON 스키마의 aem:afProperties를 통해 할당된 사용자 정의 sling:resourceType을 무시합니다. 렌더링 중에 사용자 정의 리소스 유형이 무시됩니다. (FORMS-19691)
...
* 사용자가 문서 조각에 대해 숨겨진 필드가 많은 특정 XDP를 사용하면 AEM이 cm:optional 속성을 false로 설정된 CRX 노드를 생성하여 인터랙티브 커뮤니케이션(IC) 제출이 실패합니다. (FORMS-17538)
...
* 문자를 미리 볼 때 Adobe.json 파일에서 “*” 와일드카드가 사용되는 것이 발견되어 그 목적 및 잠재적 수정에 대한 우려가 제기됩니다. (FORMS-17317)
...
* 사용자는 핵심 구성 요소를 기반으로 하는 적응형 양식에 대한 양식 속성을 저장하는 데 문제를 겪었습니다. 기초 구성 요소 편집기를 기반으로 하는 적응형 양식의 중복 스크립트가 포함되어 핵심 구성 요소를 기반으로 하는 적응형 양식에서 충돌이 발생했기 때문입니다. 편집기 (FORMS-17474)
...
* 사용자는 적응형 양식에서 장기 실행 쿼리로 인해 성능이 저하되는 문제를 경험했습니다. 이 업데이트는 효율성을 개선하기 위해 쿼리 변경 사항을 백포트합니다. 이제 고객은 aemformsAFReferences 태그 이름으로 인덱스를 생성할 수 있습니다. (FORMS-21411)
...
* 사용자는 UI에 번역되지 않은 문자열이 표시되어 인터페이스를 이해하는 데 혼란과 어려움을 겪었습니다. (FORMS-6542)
...
* 기초 구성 요소를 기반으로 하는 적응형 양식에 대한 Hcaptcha 및 Turnstile 지원이 추가되었습니다. (FORMS-16562)
* 사용자는 hCaptcha 구성 대화 상자에서 아이콘 겹침 문제를 경험했습니다. 필수 필드를 입력할 때 정보 아이콘이 오류 아이콘과 겹쳐 구성 설정 중에 혼란을 야기했습니다. (FORMS-16916)
...
* 사용자는 &#39;ReCaptchaConfigurationServiceImpl&#39;에 닫히지 않은 ResourceResolver로 인해 reCAPTCHA 기능 문제를 겪었으며, 이로 인해 양식 제출 중에 간헐적인 유효성 검사 실패가 발생했습니다. (FORMS-19241)
...
* 사용자는 Forms > 감시 폴더 만들기 > 감시 폴더 만들기 프로세스에서 현지화되지 않은 문자열을 경험했습니다. 감시 폴더를 만들 때 “감시 폴더 만들기” 및 “감시 폴더가 성공적으로 생성됨”과 같은 문자열을 찾을 수 없어 사용자 인터페이스 경험에 영향을 미쳤습니다. (FORMS-15234)
