...
* JBoss EAP 8에 대한 AEM Forms 6.5 LTS 클러스터 배포에서 `domain/configuration/domain_oracle.xml`에 잘못된 XML을 발생시키고 도메인 컨트롤러가 시작되지 않도록 하는 중복 `<security>` 태그가 더 이상 포함되지 않습니다. (FORMS-24687)
* 턴키 업그레이드 모드에서 `lc_turnkey.xml`의 데이터베이스 포트 업데이트가 업그레이드 중에 올바르게 적용되며 더 이상 이전 포트 값을 참조하지 않습니다. (FORMS-24689)
* Linux에서 JBoss EAP 8.0을 설정할 때 Windows에서 수정된 셸 스크립트는 더 이상 CRLF 줄 끝으로 인해 `/bin/sh^M: bad interpreter or $'\r': command not found` 오류가 발생하지 않습니다. (FORMS-24688)
<!--
#### Forms JEE 
...
* **FORMS-24690:** 모듈을 선택하지 않은 경우 사용자 지정 구성으로 AEM Forms 6.5 LTS JEE를 턴키 모드로 실행할 때 구성 관리자에서 부트스트랩 중에 데이터베이스 초기화가 실패합니다.

* **FORMS-24692:** 메일 서비스가 TLS 소켓 연결을 설정하지 못해 전자 메일 배달이 실패할 수 있습니다.

* Linux의 AEM Forms 6.5 LTS JEE에서 **FORMS-24741:** OSFileSetIntendedFor가 올바르게 설정되지 않은 경우 Configuration Manager가 실패할 수 있습니다. 구성 관리자를 실행하기 전에 필요한 구성 파일에서 Linux로 업데이트합니다.
