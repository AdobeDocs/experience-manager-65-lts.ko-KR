...
* 시각적 규칙 편집기의 사용자 경험이 개선되었습니다. 이번 업데이트에는 다음 사항이 포함됩니다.
  ...
  * 업데이트된 규칙 상태를 표시하려면 저장 후 요약 보기가 자동으로 다시 로드됩니다
...
* 기록 문서 다운로드(DoR)/양식 제출 서류를 규칙 편집기에 표준 단추로 추가했습니다. 이 향상된 기능을 통해 고객은 사용자 정의 코드를 작성하지 않고도 downloadDoR 기능을 사용할 수 있어 유용성과 효율성이 향상됩니다 (FORMS-21263)
...
* Sites 편집기에서 페이지 작성 중 예상되는 비헤이비어를 중단시키고 일관성 없는 구성 요소 상호 작용을 초래한 문제를 수정했습니다. 작성자가 표준 편집 작업을 방해하고 워크플로 효율성을 저하시키는 신뢰할 수 없는 UI 응답을 경험했습니다. 업데이트는 편집기 논리를 개선하고 영향을 받는 구성 요소 간에 안정적이고 예측 가능한 상호 작용을 복원합니다. (SITES-35227)
...
* 날짜 설정 및 종료 타임워프 링크에 키보드 탐색 중 초점 표시기가 표시되지 않았습니다. 이제 UI는 뚜렷하고 대비가 높은 포커스 스타일을 렌더링하므로 사용자는 활성 링크를 쉽게 식별할 수 있습니다. (SITES-25232)
* 티저 모달 헤더가 더 이상 키보드 사용자가 대화 상자를 이동할 수 없도록 차단하지 않습니다. 이제 키보드 컨트롤을 사용하여 선택, 이동 및 놓기 작업을 수행할 수 있으므로 화면 판독기 유용성과 전반적인 작동 방식을 향상시킬 수 있습니다. (SITES-25226)
...
* 페이지 업데이트 중 불규칙한 비헤이비어를 중단하도록 Sites 저작 인터페이스에서 구성 요소 처리를 조정했습니다. 이 결함으로 인해 일상적인 콘텐츠 수정을 방해하고 워크플로 효율성을 떨어뜨리는 예측 불가의 편집기 응답이 발생했습니다. 업데이트는 예상 상호 작용 패턴에 따라 편집기 논리를 정렬하고 작성 활동 중에 신뢰할 수 있는 성능을 제공합니다. (SITES-35078) 중요
...
* 론치 프로모션 범위가 작성자가 현재 페이지 및 하위 페이지 홍보를 프로모션하여 하위 섹션을 프로모션하면 표류했습니다. 또한 AEM이 관련 없는 페이지를 프로모션하여 예기치 않은 라이브-사이트를 수정했습니다. 이 수정 사항은 선택한 하위 트리만 승격되도록 Launch 범위 계산을 수정합니다. (SITES-38315)
...
* 컨텐츠 조각 시작 패키징에 새 종속성 버전과 불필요한 GraphQL 라이브러리가 있어 AEM 6.5 LTS 통합이 복잡했습니다. 이 변경 사항을 통해 종속성 버전을 AEM 6.5 LTS 기준선에 맞추고 사용하지 않는 GraphQL 종속성을 제거합니다. 이제 번들 해결이 일관되고 콘텐츠 조각 시작 이 안정적으로 유지됩니다. (SITES-35295)
* 이제 AEM Launches에서 6.5 LTS 분기에 대한 전용 Jenkins 파이프라인을 실행합니다. 이 파이프라인이 매일 밤 빌드를 실행하고 이메일로 오류 알림을 보냅니다. 이 설정은 테스트 범위를 늘리고 회귀를 조기에 식별합니다. (SITES-35293)
...
* 일반 페이지 작성 워크플로를 중단시키는 일관되지 않은 동작을 해결하기 위해 사이트 편집기가 개선되었습니다. 작성자가 예기치 않은 구성 요소 상호 작용을 하여 콘텐츠 업데이트가 방해되고 편집 안정성이 저하되었습니다. 이 변경 사항을 통해 일관된 UI 상태 관리를 복원하고 영향을 받는 시나리오 간에 작성 액션을 예측 가능하게 실행합니다. (SITES-35138)
...
이제 Assets 연결이 공백을 포함하는 파일 이름에 대해 작동합니다. 이제 업데이트된 연결 클라이언트 논리가 공간이 포함된 경로를 올바르게 처리하고 관계 선택 중 `undefined` 소스 오류를 방지합니다. 이제 연결 대화 상자가 열리고 UI 중단이나 회전기 없이 관계가 저장됩니다. DAM 사용자는 파일 이름을 변경하지 않고 자산 관계를 생성, 도출 및 제거할 수 있습니다. (Assets-56418)
...
* 사용자가 WCAG(웹 콘텐츠 접근성 지침) 감사에서 적응형 양식 실패를 경험했습니다. 양식의 탭 탐색 마크업이 유효하지 않기 때문입니다. 즉, 목록 요소가 아닌 요소는 목록의 직접 하위로 렌더링되며, 여기서 목록 항목만 허용됩니다. 이 문제로 인해 양식에서 접근성 유효성 검사기를 전달할 수 없었고 법적 또는 내부 규정 준수 요구 사항을 충족해야 하는 조직도 영향을 받았습니다. (FORMS-22101)
...
* 사용자가 적응형 양식 대시보드에서 보안 취약성을 경험했습니다. 특히 startpointcontrol.js 파일에서 악의적인 스크립트를 실행할 수 있는 XSS(크로스 사이트 스크립팅) 문제가 발견되었습니다. (FORMS-20679)
...
* 이제 턴키 모드에서 새로 설치 및 업그레이드하는 동안 데이터베이스 포트 업데이트가 올바르게 적용됩니다. 새로 설치 모드에서는 사용자가 사용 가능한 모든 포트를 선택하고 업그레이드 모드에서는 lc_turnkey.xml에서 업데이트된 데이터베이스 포트가 업그레이드 중에 올바르게 참조됩니다. (FORMS-24689)
* Linux®에서 JBoss® EAP 8.0을 설정할 때, Windows에서 수정된 셸 스크립트로 인해 더 이상 /bin/sh^M: 잘못된 인터프리터 또는 $&#39;\r&#39;: CRLF 줄 끝으로 인해 명령이 오류를 찾지 못했습니다. (FORMS-24688)
* JBoss® EAP 8에서 실행 중인 Forms JEE LTS 배포에서 Reader 확장 UI가 내부 서버 오류로 실패할 수 있습니다. (FORMS-24894)
* Linux®에서 Forms JEE LTS 구성 관리자가 설정이 해제되었거나 잘못된 `OSFileSetIntendedFor`(으)로 실행될 때 사용자에게 런타임 또는 배포 문제가 발생했습니다. `configurationManager/config/solcomp/LFS_Foundation.properties`에 값을 설정하여 Linux®에 맞게 구성이 올바르게 조정되지 않도록 했습니다. 설치 후 구성 관리자를 실행하기 전에 해당 파일에서 `OSFileSetIntendedFor=Linux`를 설정합니다. (FORMS-24741)
...
* 이제 JMX 콘솔 및 웹 콘솔에서 콘솔 CSS 리소스에 대한 Content-Type: text/css 헤더를 보냅니다. 엄격한 MIME 검사를 통해 스타일 시트 로드가 더 이상 차단되지 않으므로 `/system/console/jmx` UI가 일반적인 스타일로 렌더링됩니다. (GRANITE-63677)
...
서비스 팩 업그레이드 후 JSP 파일을 열 때 CRXDE Lite에 더 이상 빈 탭이 표시되지 않습니다. 이제 AEM에 일치하는 CodeMirror 코어 및 추가 기능 코드가 포함되어 있으므로 치명적인 브라우저 오류를 방지하고 편집기의 기능을 유지할 수 있습니다. (GRANITE-64333)
...
* Sling이 포괄적인 용어와 새로운 구성 PID를 사용하도록 관리 로그인 허용 목록을 업데이트합니다. 이 변경 사항을 통해 Sling JCR Base 3.2.0에 맞게 조정됩니다. (GRANITE-63756)
  ...
  * 더 이상 사용되지 않는 구성과 새로운 허용 목록을 동시에 실행하지 마십시오. 혼합 구성이 모호성을 만들고 의도하지 않은 비헤이비어를 생성할 수 있습니다. AEM 6.5 LTS SP2로 마이그레이션하는 경우 더 이상 사용되지 않는 PID를 제거합니다.
