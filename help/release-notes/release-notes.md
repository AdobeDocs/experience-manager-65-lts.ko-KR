...
* JBOSS EAP 8.0을 사용한 RDBMK 지원  이(가) 추가되었습니다.
...
* AEM Forms에서 기존 XML(Extensible Markup Language) 내보내기 모드를 전환하기 위해 런타임 API(Application Programming Interface)를 추가하여 -Dcom.adobe.fd.forms.export.legacy 매개 변수를 바꾸었습니다. 이러한 향상된 기능을 통해 사용자는 내보내기 모드를 보다 효율적으로 전환할 수 있으므로 워크플로 유연성이 향상됩니다. (FORMS-23115)
...
* FORMS-23971: 사용자에게 양식 데이터 모델(FDM) 편집기의 &quot;데이터 Source / 키워드 입력&quot; 기능에 문제가 발생했습니다. 이로 인해 데이터 소스를 검색하고 선택하는 기능에 영향을 주었습니다.

* FORMS-23754: 모바일 장치에서 적응형 Forms의 테이블 구성 요소가 맨 위에 숨겨진 헤더를 렌더링하여 화면 판독기가 컨텐츠를 잘못 알렸습니다. 이로 인해 화면 판독기에 의존하여 탐색하는 사용자가 영향을 받았습니다.

* FORMS-23632: 사용자가 granite:InternalArea(으)로 플래그가 지정된 리소스 유형을 참조하는 적응형 Forms 기반 적응형 구성 요소에 문제가 발생했습니다. 이는 온-프레미스 Forms 추가 기능에서 여러 granite 구성 요소의 기능에 영향을 줍니다.

* FORMS-23457: AEM 6.5 LTS SP1로 업그레이드한 후 양식을 제출할 수 없습니다. 사용자에게 com.adobe.cq.social.commons.CollabUtil이 누락되어 JSP 컴파일 오류가 발생하고 이메일 작업이 실패했습니다.

* FORMS-23426: 사용자가 적응형 Forms 기반의 기초 구성 요소에서 hCaptcha가 올바르게 번역되지 않는 문제가 발생했습니다. 이는 비영어권 사용자가 양식을 정확하게 완성할 수 있는 능력에 영향을 주었다.

* FORMS-22633: 사용자가 SAXParseException으로 인해 양식 제출 실패를 경험했습니다. &quot;컨텐츠는 프롤로그에서 허용되지 않습니다&quot;(HTTP 500). 이 문제는 미리 채우기 데이터 XML의 Null 값으로 인해 발생하여 서버측 XML 구문 분석이 실패했습니다.

* FORMS-22101: 양식의 탭-탐색 마크업이 잘못되어 목록 항목만 허용되는 목록의 직접 하위로 목록에 포함되지 않은 요소가 렌더링되었기 때문에 사용자가 적응형 Forms에서 WCAG(Web Content Accessibility Guidelines) 감사가 실패하는 것을 경험했습니다. 이로 인해 양식이 접근성 검사기를 통과하지 못했으며 법적 또는 내부 규정 준수 요구 사항을 충족해야 하는 영향을 받은 조직도 마찬가지입니다.

* FORMS-21989: 빈 양식 필드가 양식 요소로 태그가 지정되지 않은 기록 문서(DoR)/제출 PDF에서 사용자에게 접근성 문제가 발생했습니다. 이로 인해 화면 판독기에 문제가 발생하여 장애가 있는 사용자가 양식을 효과적으로 탐색하고 완료할 수 있는 기능에 영향을 미쳤습니다.

* FORMS-21925: 양식 로드 중에 하위 패널 내의 구성 요소에 대한 각주가 표시되지 않는 문제가 발생했습니다. 이 문제는 각주가 있는 항목이 페이지의 마지막 구성 요소일 때 발생했습니다.

* FORMS-21814: AEM Forms 편집기에서 구성 요소를 선택할 때 문제가 발생했습니다. 탭 사이를 이동하고 첫 번째 탭으로 돌아갈 때 일부 컨테이너는 선택할 수 없게 되어 쉽게 식별하고 상호 작용하지 못하게 되었습니다.

* FORMS-20679: 사용자가 적응형 Forms 대시보드에서 보안 취약성을 경험했습니다. 특히 startpointcontrol.js 파일에서 XSS(교차 사이트 스크립팅) 문제가 확인되었으며, 이로 인해 악의적인 스크립트가 실행될 수 있습니다.

* FORMS-24687: JBoss EAP 8에 대한 AEM Forms 6.5 LTS 클러스터 배포에서 domain/configuration/domain_oracle.xml, domain_mysql.xml 및 domain_mssql.xml 파일에 잘못된 XML을 야기하고 도메인 컨트롤러를 시작하지 못하게 하는 중복 `<security>` 태그가 더 이상 포함되어 있지 않습니다.

* FORMS-24689: 이제 턴키 모드에서 새로 설치 및 업그레이드하는 동안 데이터베이스 포트 업데이트가 올바르게 적용됩니다. 새로 설치 모드에서는 사용 가능한 모든 포트 중에서 선택할 수 있으며, 업그레이드 모드에서는 lc_turnkey.xml에서 업데이트된 데이터베이스 포트가 업그레이드 프로세스 중에 올바르게 참조됩니다.

* FORMS-24688: Linux에서 JBoss EAP 8.0을 설정할 때 Windows에서 수정된 셸 스크립트는 CRLF 줄 끝으로 인해 더 이상 `/bin/sh^M: bad interpreter or $'\r': command not found` 오류가 발생하지 않습니다.
...
* **FORMS-24690:** 모듈을 선택하지 않았거나 제한된 구성 요소만 선택한 경우 Configuration Manager에서 AEM Forms 6.5 LTS JEE 턴키 사용자 지정 모드에서 부트스트랩 중에 데이터베이스 초기화가 실패합니다. 이 오류는 종속성 누락(xalan-2.7.2.jar)으로 인해 오류가 발생했기 때문에 발생합니다. adobe-livecycle-jboss.ear\lib에 JAR 파일을 추가하면 문제가 해결됩니다.

* **FORMS-24894:** JBoss EAP 8에서 실행 중인 Forms JEE LTS 배포에서 Reader 확장 UI가 내부 서버 오류로 실패할 수 있습니다.

* **FORMS-24892:** JBoss에서 실행 중인 Forms JEE LTS에서 전자 메일 관련 기능이 실패할 수 있습니다. 전자 메일 기능을 사용하려고 할 때 서버에서 `Error IMAPProvider not a subtype`과(와) 유사한 오류를 기록할 수 있습니다.

* **FORMS-24741:** Linux 플랫폼에서 Forms JEE LTS를 사용하려면 Configuration Manager를 실행하기 전에 `OSFileSetIntendedFor`의 `LFS_Foundation.properties` 속성을 올바르게 설정해야 합니다. 업데이트되지 않으면 구성이 Linux에 맞게 제대로 조정되지 않아 런타임 또는 배포 문제가 발생할 수 있습니다. 이 문제를 해결하려면 설치 관리자를 실행한 후 Configuration Manager를 실행하기 전에 `configurationManager/config/solcomp/`(으)로 이동하여 `LFS_Foundation.properties`을(를) 열고 `OSFileSetIntendedFor=Linux`을(를) 설정하고 파일을 저장한 다음 Configuration Manager를 실행하십시오.
