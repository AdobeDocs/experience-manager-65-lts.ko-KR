...
이제 AEM 6.5 LTS SP2에 [콘텐츠 조각 및 모델 관리](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/sites/65lts/) 및 [실행](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/sites/launches/)을 위한 OpenAPI가 포함되어 있습니다. 이러한 API는 작성 및 예약을 위한 콘텐츠 조각 및 론치에 대한 액세스를 제공합니다. AEM as a Cloud Service과 동일한 최신 OpenAPI를 사용합니다.

**AEM Forms**

**AEM Forms 6.5 LTS SP2에 포함된 항목**

* JBoss® EAP 8.0으로 RDBMK 지원  이(가) 추가되었습니다.

* 시각적 규칙 편집기의 사용자 경험이 개선되었습니다. 이 업데이트에는 다음이 포함됩니다.

   * 저장 후 요약 보기를 자동으로 다시 로드하여 업데이트된 규칙 상태 표시

   * &#39;추가&#39;/&#39;삭제&#39; 단추를 표시하고 숨기는 대신 전환 허용

   * 규칙 저장 작업이 실패할 때 명확한 피드백 제공(FORMS-21261)

* `Dcom.adobe.fd.forms.export.legacy` 매개 변수를 대체하여 AEM Forms에서 기존 XML(Extensible Markup Language) 내보내기 모드를 전환하기 위해 런타임 API(Application Programming Interface)를 추가했습니다. 이러한 향상된 기능을 통해 사용자는 내보내기 모드를 보다 효율적으로 전환할 수 있으므로 워크플로 유연성이 향상됩니다. (FORMS-23115)

* 적응형 Forms에 네임스페이스 태그가 있는 JavaScript 개체 표기법(JSON)에 대한 지원이 추가되었습니다. 이 향상된 기능을 통해 사용자는 JSON 데이터 구조를 보다 효과적으로 처리할 수 있으므로 데이터 통합 및 처리 기능이 향상되었습니다. (FORMS-22519)

* 기록 문서 다운로드 (DoR) / 양식 제출 을 규칙 편집기에 기본 제공 (OOTB) 버튼으로 추가했습니다. 이러한 개선된 기능을 통해 고객이 사용자 지정 코드를 작성하지 않고도 downloadDoR 기능을 사용할 수 있어 유용성과 효율성이 향상됐다. (FORMS-21263)

* 적응형 Forms에 네임스페이스 태그가 있는 JavaScript 개체 표기법(JSON)에 대한 지원이 추가되었습니다. 이러한 향상된 기능을 통해 사용자는 양식을 보다 정확하고 효율적으로 미리 채울 수 있으므로 데이터 통합을 향상시키고 수동 입력 오류를 줄일 수 있습니다. (FORMS-10883)
...
* 페이지 편집기에서 자산 선택기를 손상시켜 특정 페이지 편집 시나리오에서 로드할 수 없는 회귀입니다. 이제 작성자는 페이지를 편집하는 동안 에셋을 선택하거나 탐색할 때 에셋 선택기를 정상적으로 열고 사용할 수 있습니다. 이 변경 사항은 로드 실패가 중단되었던 자산 선택 워크플로우에 대한 일관된 액세스 권한을 복원합니다. (SITES-35226)
...
* 티저 모달 헤더가 더 이상 키보드 사용자가 대화 상자를 이동할 수 없도록 차단하지 않습니다. 이제 키보드 컨트롤을 사용하여 픽업, 이동 및 놓기 작업을 수행할 수 있으므로 화면 판독기의 유용성과 전반적인 조작성을 향상시킬 수 있습니다. (SITES-25226)
...
AEM Headless에는 메인 라인 빌드와 종속성 및 번들 버전 충돌을 방지하기 위해 전용 릴리스 분기가 필요했습니다. 업데이트에서는 `release/6.5lts` 헤드리스 분기를 추가하고 종속성 집합 및 번들 버전을 정렬합니다. 이제 Jenkins는 버전 충돌 없이 Headless 코드 베이스를 깔끔하게 빌드합니다. (SITES-36585)
...
* 이제 Universal Editor Open 명령은 개발 및 스테이징 환경에서 기본적으로 미리보기 모드로 설정됩니다. 명령이 `preview=true`을(를) 추가하여 작성자 검사를 미리 보기 컨텍스트와 정렬하고 실수로 프로덕션이 열리지 않도록 합니다. (SITES-33839)
...
* 사용자에게 양식 데이터 모델(FDM) 편집기 `Data Source / Enter Keyword` 기능에 문제가 발생했습니다. 이 문제는 데이터 소스 검색 및 선택 기능에 영향을 주었습니다. (FORMS-23971)
* 모바일 장치에서 적응형 Forms의 표 구성 요소는 맨 위에 숨겨진 헤더를 렌더링하여 화면 판독기에서 콘텐츠를 잘못 알리게 했습니다. 이로 인해 화면 판독기에 의존하여 탐색하는 사용자가 영향을 받았습니다. (FORMS-23754)
* 사용자에게 granite:InternalArea(으)로 플래그가 지정된 리소스 유형을 참조하는 적응형 Forms 기반의 핵심 구성 요소와 관련된 문제가 발생했습니다. 이는 온-프레미스 Forms 추가 기능에 있는 여러 granite 구성 요소의 기능에 영향을 줍니다. (FORMS-23632)
* AEM 6.5 LTS SP1로 업그레이드한 후 양식을 제출하지 못했습니다. 사용자에게 com.adobe.cq.social.commons.CollabUtil이 누락되어 JSP 컴파일 오류가 발생하고 이메일 작업이 실패했습니다. (FORMS-23457)
* 사용자가 적응형 Forms 기반의 기초 구성 요소에서 hCaptcha가 올바르게 번역되지 않는 문제가 발생했습니다. 이는 비영어권 사용자가 양식을 정확하게 완성할 수 있는 능력에 영향을 주었다. (FORMS-23426)
* 사용자가 SAXParseException으로 인해 양식 제출 오류를 경험했습니다. &quot;컨텐츠는 프롤로그에서 허용되지 않습니다.&quot;(HTTP 500). 이 문제는 미리 채우기 데이터 XML의 Null 값으로 인해 발생하여 서버측 XML 구문 분석이 실패했습니다. (FORMS-22633)
* 사용자가 WCAG(Web Content Accessibility Guidelines) 감사에서 적응형 Forms 실패를 경험했습니다. 그 이유는 양식의 탭 탐색 마크업이 잘못되었기 때문입니다. 즉, 목록 요소가 아닌 요소는 목록의 직접 하위로 렌더링되며, 여기서 목록 항목만 허용됩니다. 이 문제로 인해 양식에서 접근성 검사기를 전달할 수 없었고 법적 또는 내부 규정 준수 요구 사항을 충족해야 하는 영향을 받는 조직도 마찬가지입니다. (FORMS-22101)
* 사용자에게 기록 문서(DoR)/제출 PDF에서 빈 양식 필드에 양식 요소로 태그가 지정되지 않은 접근성 문제가 발생했습니다. 이로 인해 화면 판독기에 문제가 발생하여 장애가 있는 사용자가 양식을 효과적으로 탐색하고 완료할 수 있는 기능에 영향을 미쳤습니다. (FORMS-21989)
...
* 사용자가 적응형 Forms 대시보드에서 보안 취약점이 발생했습니다. 특히 startpointcontrol.js 파일에서 XSS(교차 사이트 스크립팅) 문제가 확인되었으며, 이로 인해 악의적인 스크립트가 실행될 수 있습니다. (FORMS-20679)
* JBoss® EAP 8에 대한 AEM Forms 6.5 LTS 클러스터 배포에서 `domain/configuration/domain_oracle.xml`, `domain_mysql.xml` 및 `domain_mssql.xml` 파일에 잘못된 XML을 야기하고 도메인 컨트롤러를 시작하지 못하게 하는 중복 `<security>` 태그가 더 이상 포함되어 있지 않습니다. (FORMS-24687)
* 이제 턴키 모드에서 새로 설치 및 업그레이드하는 동안 데이터베이스 포트 업데이트가 올바르게 적용됩니다. 새로 설치 모드에서는 사용 가능한 모든 포트 중에서 선택할 수 있으며, 업그레이드 모드에서는 lc_turnkey.xml에서 업데이트된 데이터베이스 포트가 업그레이드 프로세스 중에 올바르게 참조됩니다. (FORMS-24689)
* Linux®에서 JBoss® EAP 8.0을 설정할 때 Windows에서 수정된 셸 스크립트는 더 이상 CRLF 줄 끝으로 인해 `/bin/sh^M: bad interpreter or $'\r': command not found` 오류가 발생하지 않습니다. (FORMS-24688)
...
* Felix 웹 콘솔은 `/system/console` 액세스 중에 더 이상 간헐적인 `NullPointerException` 오류를 발생시키지 않습니다. 업데이트된 ServiceTracker 처리로 인해 null 추적기 상태가 방지됩니다. 콘솔 로그인 및 탐색은 반복된 요청 및 자동화된 유효성 검사 동안 안정적으로 유지됩니다. (GRANITE-61042)
...
* Sling은 포괄적인 용어와 새로운 PID를 사용하도록 관리 로그인 구성을 업데이트합니다. 이 변경 사항은 Sling JCR Base 3.2.0에 맞게 조정됩니다. (GRANITE-63756)
...
이러한 이전 구성은 `whitelist.name` 및 `whitelist.bundles`과(와) 같은 속성을 사용합니다.
   ...
   * 더 이상 사용되지 않는 구성과 새로운 구성을 동시에 실행하지 마십시오. 혼합 구성은 모호성을 만들고 의도하지 않은 비헤이비어를 생성할 수 있습니다. AEM 6.5 LTS SP2로 마이그레이션하는 경우 더 이상 사용되지 않는 PID를 완전히 제거하십시오.

  **수행할 작업**

   1. `LoginAdminWhitelist*` PID를 사용하는 PID 구성 찾기
...
AEM 6.5 LTS SP2용 UberJar는 AEM 6.5 LTS UberJar 버전 6.6.2를 사용합니다. Maven 중앙 저장소에서 해당 UberJar 아티팩트를 검색할 수 있습니다. AEM 6.5와 달리 AEM 6.5 LTS는 공개 API와 더 이상 사용되지 않는 API를 두 개의 서로 다른 아티팩트로 구분합니다.

공용 API에 대해 컴파일하려면 다음을 사용하십시오.
...
[AEM Uber Jar 버전 업데이트](/help/sites-deploying/upgrading-code-and-customizations.md#update-the-aem-uber-jar-version)도 참조하세요.
...
고객은 현재 배포에서 이 기능/기능을 사용하는지 검토하는 것이 좋습니다. 제공된 대체 요소를 사용하도록 구현을 변경할 계획을 세우십시오.
...
| Sites | [SPA 편집기](/help/sites-developing/spa-overview.md) | AEM에서 Headless 콘텐츠를 관리하기 위한 기본 편집기는 다음과 같습니다. <br>- [시각적 편집을 위한 범용 편집기](/help/sites-developing/universal-editor/introduction.md).<br>- [콘텐츠 조각 편집기](/help/assets/content-fragments/content-fragments-managing.md). | 6.5 LTS GA |
...
* Configuration Manager에서 모듈을 선택하지 않았거나 제한된 구성 요소만 선택한 경우 AEM Forms 6.5 LTS JEE 턴키 사용자 지정 모드의 Bootstrap 중에 데이터베이스 초기화가 실패합니다. 이 실패는 종속성 누락(xalan-2.7.2.jar)으로 인해 오류가 발생합니다. adobe-livecycle-jboss.ear\lib에 JAR 파일을 추가하면 문제가 해결됩니다. (FORMS-24690)
* JBoss® EAP 8에서 실행 중인 Forms JEE LTS 배포에서 Reader 확장 UI가 내부 서버 오류로 실패할 수 있습니다. (FORMS-24894)
* JBoss®에서 실행 중인 Forms JEE LTS에서 이메일 관련 기능이 실패할 수 있습니다. 전자 메일 기능을 사용하려고 할 때 서버에서 `Error IMAPProvider not a subtype`과(와) 유사한 오류를 기록할 수 있습니다. (FORMS-24892)
* Linux® 플랫폼에서 Forms JEE LTS를 사용하려면 구성 관리자를 실행하기 전에 `LFS_Foundation.properties`의 `OSFileSetIntendedFor` 속성을 올바르게 설정해야 합니다. 업데이트되지 않으면 구성이 Linux®에 맞게 제대로 조정되지 않아 런타임 또는 배포 문제가 발생할 수 있습니다. 이 문제를 해결하려면 설치 관리자를 실행한 후 Configuration Manager를 실행하기 전에 `configurationManager/config/solcomp/`(으)로 이동하여 `LFS_Foundation.properties`을(를) 열고 `OSFileSetIntendedFor=Linux`을(를) 설정하고 파일을 저장한 다음 Configuration Manager를 실행하십시오. (FORMS-24741)
...
이전에 JCR 저장소에서 오프라인 압축을 실행한 경우 온라인 압축 중에 저장소 손상이 발생할 수 있습니다. 이 시나리오에서는 `SegmentNotFoundException`(SNFE)이 발생할 수 있으며 이로 인해 리포지토리가 손상될 수 있습니다.

이 문제를 해결하려면 [소프트웨어 배포](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.2-hotfix-GRANITE-65388-1.0.zip)에서 핫픽스를 설치하십시오. 핫픽스에 낮은 수준의 `oak-segment-tar` 번들이 포함되어 있으므로 설치 후 인스턴스가 다시 시작됩니다.

인스턴스 적용 시 다운타임에 대한 계획을 수립합니다. 오프라인 압축을 위해 소프트웨어 배포에서도 사용할 수 있는 해당 [`oak-run` jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.88.1-B006.jar)을(를) 사용합니다.

>[!NOTE]
>
> * `oak-run` 작업의 경우 [`oak-run` 1.88.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.88.1-B006.jar)을(를) 사용하십시오.
>
> * 시스템 속성 `oak.compaction.legacy=true`을(를) 설정하여 AEM을 시작합니다.
...
* HTTP 400 응답 코드를 사용한 상태 검사 오류.
* Dispatcher 및 AEM 인스턴스 간의 트래픽이 중단되었습니다.
* Dispatcher을 통해 콘텐츠를 제대로 제공할 수 없습니다.
* Dispatcher 구성에서 IP 주소가 있는 HTTPS를 사용할 때 연결 오류가 발생합니다.
* HTTPS + IP를 통해 연결할 때 HTTP 400 &quot;잘못된 SNI&quot; 오류.

**영향을 받는 환경**

* Dispatcher 구성을 사용한 AEM 배포.
* SSL 전용 기능이 활성화된 시스템입니다.
* AEM 인스턴스에 대한 `https + IP` 연결 메서드를 사용하는 Dispatcher 구성입니다.
...
다음 텍스트 문서에는 이 [!DNL Experience Manager] 6.5 LTS, 서비스 팩 2 릴리스에 포함된 OSGi 번들 및 컨텐츠 패키지 목록이 나와 있습니다. <!-- UPDATE FOR EACH NEW RELEASE -->

* [Experience Manager 6.5 LTS, 서비스 팩에 포함된 OSGi 번들 목록](/help/release-notes/assets/65lts_sp2_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Experience Manager 6.5 LTS, 서비스 팩에 포함된 콘텐츠 패키지 목록](/help/release-notes/assets/65lts_sp2_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
