...
* 페이지 편집 중에 일관되지 않은 동작을 유발하고 일반 워크플로우를 중단시키는 사이트 작성 인터페이스의 결함을 제거했습니다. 작성자가 구성 요소 상호 작용 및 콘텐츠 업데이트를 방해하는 예기치 않은 UI 응답을 발견했습니다. 이 업데이트는 영향을 받는 기능을 안정화하고 편집 시나리오 간에 안정적이고 예측 가능한 동작을 복원합니다. (SITES-35224)
* 이제 AEM Sites에서 ADA 및 WCAG 요구 사항을 충족하기 위해 이미지에 대한 `alt` 텍스트 지원이 포함됩니다. 페이지 출력에서 더 이상 `alt` 특성이 생략되지 않으므로 화면 판독기에서 올바른 대체 텍스트를 받습니다. (SITES-27153)
...
* 이제 카드 작업 메뉴 표시줄에 카드 보기에 액세스 가능한 이름이 표시됩니다. 화면 판독기는 메뉴 막대 목적을 명확하게 알려주고, 음성 제어는 이름별로 제어를 대상으로 할 수 있습니다. (SITES-24938)
...
* 이미지 구성 요소 대화 상자에서는 더 이상 장식용 도움말 및 전체 화면 아이콘을 이미지로 표시하지 않습니다. 이제 화면 판독기는 이러한 아이콘을 건너뛰고 실행 가능한 컨트롤 및 필드 내용에 초점을 맞춥니다. (SITES-2940)
...
사이트 콘솔 목록 보기 설정에 목록 보기에 표시된 열이 반영되지 않았습니다. 확인란 선택을 취소하고 선택한 열의 수가 잘못된 상태로 대화 상자가 열립니다. 수정 대화 상자는 활성 그리드 열과 상태를 동기화하고 실제 열 가시성과 일치하도록 카운터를 업데이트합니다. (SITES-38576)
...
* 리팩터링 후 기능 토글이 비활성화된 상태로 유지되면 콘텐츠 조각 변형 태그가 사라집니다. 수정 사항은 토글이 꺼져 있는 경우에도 변형 태그 지원을 복원합니다. 작성자는 콘텐츠 조각 편집기에서 변형 태그를 다시 추가하고 볼 수 있습니다. (사이트-38682) 위험
...
* 콘텐츠 조각 모델 필터 패널이 더 이상 현지화되지 않은 문자열을 표시하지 않습니다. 이제 AEM에 모든 로케일에서 지역화된 필터 레이블 및 지역화된 상태 값이 표시됩니다. (SITES-30863)
* 이제 콘텐츠 조각 모델 편집기에서 잠금 경고 대화 상자의 지역화된 문자열을 렌더링합니다. UI는 지역화되지 않은 영어 메시지를 지원되는 언어의 로케일 리소스로 대체합니다. (SITES-28592)

#### [!DNL Content Fragments] - REST API{#sites-restapi-65-lts-sp2}

AEM Headless에는 메인 라인 빌드와 종속성 및 번들 버전 충돌을 방지하기 위해 전용 릴리스 분기가 필요했습니다. 이 업데이트에서는 릴리스/6.5lts Headless 분기를 추가하고 종속성 세트 및 번들 버전을 정렬합니다. Jenkins는 이제 버전 충돌 없이 헤드리스 코드베이스를 깨끗하게 빌드합니다. (SITES-36585)
...
* 표준 페이지 편집 작업 과정을 방해하는 일관되지 않은 동작을 해결하기 위해 사이트 작성 환경이 변경되었습니다. 작성자가 구성 요소 상호 작용 중에 예기치 않은 결과가 발생하여 콘텐츠 업데이트가 중단되고 신뢰성이 저하되었습니다. 이 변경은 안정적인 편집기 동작을 복원하고 영향을 받는 시나리오 간에 작성 동작이 일관되게 실행되도록 합니다. (사이트-35162) 위험

* 페이지 편집을 방해하고 구성 요소 상호 작용 중에 일관성 없는 결과를 초래하는 문제를 해결하기 위해 사이트 작성 동작을 개선했습니다. 작성자가 콘텐츠 업데이트를 방해하고 워크플로우 안정성을 감소시키는 예기치 않은 UI 응답을 경험했습니다. 이 변경 사항은 안정적인 편집기 상태 관리를 복원하고 영향을 받는 시나리오 간에 작성 작업을 예측 가능하게 실행합니다. (SITES-34499)
...
* AEM 6.5 LTS Quickstart에는 필요한 론치 번들 및 사전 요구 사항이 없어서 론치 OpenAPI 활성화가 차단되었습니다. 업데이트에는 론치 번들 및 필요한 종속성(예: 지표 지원, DAM-cfm 업데이트 및 큐 구성)이 추가됩니다. 필요한 런타임 구성 요소가 있는 6.5 LTS Quickstart에서 지금 실행되는 API를 실행합니다. (SITES-35297)
* CF는 AEM 6.5 LTS 통합이 복잡했던 최신 종속성 버전과 불필요한 GraphQL 라이브러리를 패키징하기 시작합니다. 이 변경 사항은 AEM 6.5 LTS 기준에 따라 종속성 버전을 정렬하고 사용하지 않은 GraphQL 종속성을 제거합니다. 이제 번들 해결은 일관되고 CF Launches 시작은 안정적으로 유지됩니다. (SITES-35295)
...
이제 페이지 편집기를 게시하면 선택기 또는 접미사가 포함된 URL이 처리됩니다. 이제 게시된 요청은 선택기나 접미어 URL 문자열이 아닌 JCR 페이지 경로를 보내므로 활성화가 완료되고 콘텐츠가 공개됩니다. 이제 복제 시 오류 상태가 반환되어 false &quot;게시 시작됨&quot; 메시지가 표시되지 않습니다. (NPR-43288)

<!-- #### Rich Text Editor{#sites-rte-65-lts-sp2} -->

#### 템플릿 편집기{#sites-template-editor-65-lts-sp2}

일부 로케일의 **도구** > **일반** > **템플릿**&#x200B;에 템플릿 상태 텍스트가 세로로 표시됩니다. &quot;오래됨&quot; 레이블이 레이아웃을 깨고 문자 열로 읽혔습니다. 수정은 레이블이 단일 가로 라인에서 렌더링되도록 템플릿 상태 스타일을 수정합니다. (SITES-36797)
...
* 이제 Universal Editor Open 명령은 Dev 및 Stage 환경의 Preview 모드로 기본 설정됩니다. 이 명령은 preview=true를 추가하므로 작성자 확인이 미리 보기 컨텍스트에 맞게 정렬되고 실수로 [프로덕션]이 열리지 않도록 합니다. (SITES-33839)

### [!DNL Assets]{#assets-65-lts-sp2}

이제 관련 에셋이 공백이 포함된 파일 이름에 대해 작동합니다. 업데이트된 관계 클라이언트 논리는 이제 공간이 포함된 경로를 올바르게 처리하고 관계를 선택하는 동안 `undefined` 소스 오류를 방지합니다. 이제 연관(Relate) 대화상자가 열리고 UI 중단이나 스피너 없이 관계식이 저장됩니다. DAM 사용자는 파일 이름을 바꾸지 않고도 에셋을 연결하고, 파생하고, 연결 해제할 수 있습니다. (Assets-56418)
...
* AEM은 번들 새로 고침 작업 중에 더 이상 방화벽 허용 목록에 추가하다 및 차단 목록에 추가하다 deserialization 설정을 지우지 않습니다. 업데이트된 필터 등록 로직은 활성 방화벽 인스턴스를 저장된 구성에 맞춰 유지하므로 재시작 없이 보호가 활성화된 상태를 유지합니다. (GRANITE-61382)
* Felix Web Console은 `NullPointerException` 액세스 중에 더 이상 간헐적인 `/system/console` 오류를 발생시키지 않습니다. 업데이트된 ServiceTracker 처리로 Null 추적기 상태가 방지됩니다. 콘솔 로그인 및 탐색은 반복된 요청 및 자동화된 유효성 검사 동안 안정적으로 유지됩니다. (GRANITE-61042)
...
AEM에는 이제 Sling Engine 2.16.6이 포함됩니다. 이 변경은 보안 도구로 플래그가 지정된 XSS 위반을 제거하고 핵심 렌더링 안전성과 안정성을 개선합니다. (NPR-43105)
...
EmailNotificationService-Processor는 더 이상 워크플로 알림 처리 중 반복되는 &quot;세그먼트를 찾을 수 없음&quot; 오류를 트리거하지 않습니다. 업데이트된 예외 처리는 SegmentNotFoundException을 감지하고 잘못된 읽기를 계속하는 대신 처리 루프를 중지합니다. 작업 과정 실행은 안정적인 상태로 유지되며 받은 편지함 및 작업 항목 액세스 중에 로그 노이즈가 사라집니다. (GRANITE-62635)
...
AEM 6.5 LTS SP2용 UberJar는 AEM 6.5 LTS UberJar 버전 6.6.0을 사용합니다. Maven 중앙 저장소에서 해당 UberJar 아티팩트를 검색할 수 있습니다. AEM 6.5와 달리 AEM 6.5 LTS는 공개 API와 더 이상 사용되지 않는 API를 두 개의 서로 다른 아티팩트로 구분합니다.
...
Adobe은 제품 기능을 지속적으로 검토하고 발전시켜 기존 기능을 현대화하거나 대체함으로써 더 큰 고객 가치를 제공합니다. 이러한 변경 사항은 이전 버전과의 호환성을 신중하게 고려하여 구현됩니다.
...
* 제거는 다음 주요 릴리스보다 먼저 수행됩니다. 계획된 제거 타임라인은 별도로 전달됩니다.
