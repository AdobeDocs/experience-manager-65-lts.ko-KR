...
다음 표에는 연결 모드에 종속된 JAR 파일과 AEM Forms이 배포되는 J2EE 응용 프로그램 서버가 나열되어 있습니다.
...
1. `DSC_DEFAULT_EJB_ENDPOINT` 연결 속성을 설정하려면 `java.util.Properties` 개체의 `setProperty` 메서드를 호출하고 다음 값을 전달하십시오.
   ...
   * AEM Forms을 호스팅하는 J2EE 애플리케이션 서버의 URL을 지정하는 문자열 값
   ...
   >[!NOTE]
   >
   >SOAP 연결 모드를 사용하는 경우 `ServiceClientFactoryProperties.DSC_DEFAULT_SOAP_ENDPOINT` 열거형 값 대신 `ServiceClientFactoryProperties.DSC_DEFAULT_EJB_ENDPOINT` 열거형 값을 지정하십시오.
...
1. `DSC_TRANSPORT_PROTOCOL` 연결 속성을 설정하려면 `java.util.Properties` 개체의 `setProperty` 메서드를 호출하고 다음 값을 전달하십시오.
   ...
   * `ServiceClientFactoryProperties.DSC_EJB_PROTOCOL` 열거형 값
   ...
   >[!NOTE]
   >
   >SOAP 연결 모드를 사용하는 경우 `ServiceClientFactoryProperties.DSC_SOAP_PROTOCOL` 열거형 값 대신 `ServiceClientFactoryProperties.DSC_EJB_PROTOCOL`열거형 값을 지정하십시오.
...
다음 코드 예제에서는 `com.adobe.idp.Context` 개체를 만드는 데 사용되는 연결 설정 내에서 `EncryptionServiceClient` 개체를 사용하는 방법을 보여 줍니다.
...
다음 코드 예제에서는 `com.adobe.idp.Document`map.pdf *이라는 PDF 파일을 기반으로 하는* 개체를 만듭니다. 이 파일은 C 하드 드라이브의 루트에 있습니다. 이 생성자는 파일 이름 확장명을 사용하여 `com.adobe.idp.Document` 개체의 MIME 콘텐츠 형식을 설정하려고 합니다.

`com.adobe.idp.Document` 개체를 허용하는 `java.io.File` 생성자도 부울 매개 변수를 허용합니다. 이 매개 변수를 `true`(으)로 설정하면 `com.adobe.idp.Document` 개체가 파일을 삭제합니다. 이 작업은 파일을 `com.adobe.idp.Document` 생성자에 전달한 후 제거할 필요가 없음을 의미합니다.
...
다음 Java 코드 예제에서는 `com.adobe.idp.Document` 개체를 기반으로 하는 `java.io.InputStream` 개체를 만듭니다.
...
다음 Java 코드 예제에서는 `com.adobe.idp.Document`map.pdf *이라는 PDF 파일을 기반으로 하는* 개체를 만듭니다. 이 파일은 `WebApp`에서 실행 중인 웹 응용 프로그램 `localhost`에 있습니다. 이 생성자는 URL 프로토콜과 함께 반환된 콘텐츠 형식을 사용하여 `com.adobe.idp.Document` 개체의 MIME 콘텐츠 형식을 설정하려고 합니다.
...
`com.adobe.idp.Document` 개체의 `com.adobe.idp.Document` 메서드를 호출하여 `getContentType` 개체의 MIME 형식을 확인하십시오. 이 메서드는 `com.adobe.idp.Document` 개체의 콘텐츠 형식을 지정하는 문자열 값을 반환합니다. 다음 표에서는 AEM Forms이 반환하는 다양한 콘텐츠 유형을 설명합니다.
...
연결 속성을 설정한 후 서비스를 호출할 수 있는 Java 개체를 인스턴스화하는 데 사용되는 `ServiceClientFactory` 개체를 만듭니다. Java 클라이언트 라이브러리가 있는 각 서비스에는 해당 클라이언트 개체가 있습니다. 예를 들어 Repository 서비스를 호출하려면 해당 생성자를 사용하고 `ResourceRepositoryClient` 개체를 전달하여 `ServiceClientFactory` 개체를 만듭니다. `ServiceClientFactory` 개체는 AEM Forms 서비스를 호출하는 데 필요한 연결 설정을 유지 관리합니다.
...
`com.adobe.idp.Context` 개체에 영향을 주는 `ServiceClientFactory` 개체 내에 있는 SAML 어설션의 수명을 제어하는 사용자 관리자 설정이 있습니다. 이 설정은 Java API를 사용하여 수행되는 모든 호출을 포함하여 AEM Forms 전체의 모든 인증 컨텍스트 수명을 제어합니다. 기본적으로 `ServiceCleintFactory` 개체를 사용할 수 있는 기간은 2시간입니다.
...
1. `ServiceClientFactory` 개체의 정적 `ServiceClientFactory` 메서드를 호출하고 연결 속성이 포함된 `createInstance` 개체를 전달하여 `java.util.Properties` 개체를 만듭니다.
1. 생성자를 사용하고 `ResourceRepositoryClient` 개체를 전달하여 `ServiceClientFactory` 개체를 만듭니다. `ResourceRepositoryClient` 개체를 사용하여 저장소 서비스 작업을 호출하십시오.
1. 생성자를 사용하여 `RepositoryInfomodelFactoryBean` 개체를 만들고 `null`을(를) 전달합니다. 이 개체를 사용하면 저장소에 추가된 콘텐츠를 나타내는 `Resource` 개체를 만들 수 있습니다.
1. `Resource` 개체의 `RepositoryInfomodelFactoryBean` 메서드를 호출하고 다음 값을 전달하여 `newImage` 개체를 만듭니다.
...
1. `ResourceContent` 개체의 `RepositoryInfomodelFactoryBean` 메서드를 호출하고 반환 값을 `newImage`(으)로 캐스팅하여 `ResourceContent` 개체를 만듭니다. 이 객체는 저장소에 추가된 컨텐츠를 나타냅니다.
1. 저장소에 추가할 XDP 파일을 저장하는 `com.adobe.idp.Document` 개체를 전달하여 `java.io.FileInputStream` 개체를 만듭니다. [InputStream 개체를 기반으로 문서 만들기](invoking-aem-forms-using-java.md#creating-a-document-based-on-an-inputstream-object)를 참조하십시오.
1. `com.adobe.idp.Document` 개체의 `ResourceContent` 메서드를 호출하여 `ResourceContent` 개체의 내용을 `setDataDocument` 개체에 추가합니다. `com.adobe.idp.Document` 개체를 전달합니다.
1. `ResourceContent` 개체의 `setMimeType` 메서드를 호출하고 `application/vnd.adobe.xdp+xml`을(를) 전달하여 저장소에 추가할 XDP 파일의 MIME 형식을 설정하십시오.
1. `ResourceContent` 개체의 `Resource` 메서드를 호출하고 `Resource` 개체를 전달하여 `setContent` 개체의 내용을 `ResourceContent` 개체에 추가하십시오.
...
1. 프로세스에 전달되는 안전하지 않은 PDF 문서를 가져옵니다. 이 작업은 `SetValue` 작업을 기반으로 합니다. 이 프로세스에 대한 입력 매개 변수는 이름이 `document`인 `inDoc` 프로세스 변수입니다.
...
1. 생성자를 사용하고 `ServiceClient` 개체를 전달하여 `ServiceClientFactory` 개체를 만듭니다. `ServiceClient` 개체를 사용하면 서비스 작업을 호출할 수 있습니다. 호출 요청 찾기, 발송 및 라우팅 등의 작업을 처리합니다.
...
1. `InvocationRequest` 개체의 `ServiceClientFactory` 메서드를 호출하고 다음 값을 전달하여 `createInvocationRequest` 개체를 만듭니다.
...
1. `ServiceClient` 개체의 `invoke` 메서드를 호출하고 `InvocationRequest` 개체를 전달하여 서비스에 호출 요청을 보냅니다. `invoke` 메서드가 `InvocationReponse` 개체를 반환합니다.
   ...
   >[!NOTE]
   >
   >`false` 메서드의 네 번째 매개 변수로 `createInvocationRequest` 값을 전달하여 장기 프로세스를 호출할 수 있습니다. `false`*값을 전달하면 비동기 요청이 만들어집니다.*
...
1. `com.adobe.idp.Document` 개체의 `copyToFile` 메서드를 호출하여 `com.adobe.idp.Document` 개체의 내용을 파일에 복사합니다. `com.adobe.idp.Document` 메서드에서 반환된 `getOutputParameter` 개체를 사용하는지 확인하십시오.
