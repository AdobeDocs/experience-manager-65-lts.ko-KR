...
1. 인증서 속성 중 하나를 사용자 속성에 매핑합니다. 예를 들어 인증서의 일반 이름을 사용자의 로그인 ID에 매핑할 수 있습니다.
   ...
   인증서의 속성 내용이 사용자 관리 데이터베이스의 사용자 속성 내용과 다른 경우 Java 정규 표현식(regex)을 사용하여 두 속성을 일치시킬 수 있습니다. 예를 들어, 인증서의 일반 이름이 *Alex Pink(인증)* 및 *Alex Pink(서명)*&#x200B;이고 사용자 관리 데이터베이스의 일반 이름이 *Alex Pink*&#x200B;인 경우 정규 표현식을 사용하여 인증서 특성의 필요한 부분을 추출합니다(이 예제에서는 *Alex Pink*). 지정하는 정규 표현식은 Java 정규 표현식 사양을 준수해야 합니다.
