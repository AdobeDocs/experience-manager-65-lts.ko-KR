...
새 패키지 버전을 설치하려면 4.1 및 4.2에 정의된 단계를 따릅니다. 이전 버전이 이미 설치된 경우에도 최신 패키지 버전을 설치할 수 있습니다. 그러나 Adobe에서는 먼저 이전 패키지 버전을 제거하는 것이 좋습니다. 이렇게 하려면 다음을 수행합니다.
...
1. 확인 시 **제거**&#x200B;를 다시 선택하고 제거 프로세스를 완료하도록 허용하십시오.
...
1. **forms 사용자**&#x200B;를 검색하고, **forms-power-users** 그룹을 선택하고, **We.Gov Forms 사용자** 그룹을 멤버로 추가하여 2~7단계를 반복합니다.
1. **forms-users**&#x200B;를 검색하고 **forms-users** 그룹을 선택한 다음 이번에는 **We.Gov Users** 그룹을 멤버로 추가하여 2~7단계를 반복합니다.
...
1. **다음**&#x200B;을 클릭합니다.
1. HTTPS 호스트 이름을 *localhost*(으)로 설정합니다.
1. Port 를 시스템이 공개한 포트로 설정합니다.
...
1. 다음으로 이동합니다.
   ...
   *https://&lt;aemserver>:&lt;port>/libs/adobesign/cloudservices/adobesign.html/conf/we-gov*
   ...
   >[!NOTE]
   >
   >구성 문제가 발생하지 않도록 하려면 AEM 서버에 액세스하는 데 사용된 URL이 Adobe Sign OAuth 리디렉션 URI에 구성된 URL과 일치하는지 확인하십시오.
   >예: *https://&lt;aemserver>:&lt;port>/mnt/overlay/adobesign/cloudservices/adobesign/properties.html*

1. **We.gov Adobe Sign** 구성을 선택하십시오.
1. **속성**&#x200B;을 클릭합니다.
1. &quot;설정&quot; 탭으로 이동합니다.
1. oAuth URL을 입력합니다. 예: `https://secure.na1.echosign.com/public/oauth`(일러스트레이션 목적으로만 사용)
1. 구성된 Adobe Sign 인스턴스에서 구성된 클라이언트 ID 및 클라이언트 암호를 제공합니다.
1. **Adobe Sign에 연결**&#x200B;을 클릭합니다.
1. 연결이 성공하면 **저장 및 닫기**&#x200B;를 클릭하여 통합을 완료합니다.
...
1. **설정** 또는 **매니페스트**&#x200B;를 클릭하여 **회신 URL을 구성하세요.**
...
1. 오른쪽 아래에서 **사용자 지정** 및 **개발자 리소스**&#x200B;를 클릭합니다.
1. 서비스 루트 URL을 찾습니다. 예:

   `https://msdynamicsserver.api.crm3.dynamics.com/api/data/v9.1/`

1. 새로 고침 및 액세스 토큰 URL에 대한 자세한 내용은 다음에서 확인할 수 있습니다.
...
1. **컨텍스트 인식 구성**&#x200B;이(가) `/conf/we-gov`(으)로 설정되어 있고 구성된 데이터 원본이 `ms-dynamics-odata-cloud-service`인지 확인하십시오.
...
1. 서비스를 테스트하여 구성된 데이터 소스에 성공적으로 연결되었는지 확인합니다.
   ...
   >[!NOTE]
   >
   >데이터 소스가 FDM에 성공적으로 바인딩되려면 AEM 서버를 다시 시작해야 한다고 보고되었습니다.

   >[!NOTE]
   >
   >Adobe에서는 `Ctrl + C` 명령을 사용하여 SDK을 다시 시작할 것을 권장합니다. 대체 방법을 사용하여 AEM SDK을 다시 시작합니다. 예를 들어 Java 프로세스를 중지하면 AEM 개발 환경이 일치하지 않을 수 있습니다.

#### Forms 데이터 모델 테스트(Derby) {#test-fdm-derby}

클라우드 구성이 완료되면 Forms 데이터 모델을 테스트할 수 있습니다.
...
계정 관리자가 다음 작업을 수행해야 하는 Adobe Analytics 계정에 대한 사용자 자격 증명을 찾습니다.
...
1. 사용자 이름 및 공유 암호 정보가 권한 상자의 오른쪽에 나타납니다.
1. 사용자 이름은 이름 내에 콜론을 포함합니다. 다음 예와 같이 콜론 왼쪽에 있는 모든 정보는 사용자 이름이며 콜론 오른쪽에 있는 모든 정보는 회사 이름입니다.
...
>[!NOTE]
>
>`we-gov-forms.ui.analytics-<version>.zip` 패키지를 설치하는 경우 AEM Forms Analytics 데이터는 오프라인에서 사용하거나 Adobe Analytics Cloud 구성 없이 사용할 수 있습니다. AEM Sites 데이터에는 활성 클라우드 구성이 필요합니다.
...
1. 이 페이지에서는 Adobe Analytics에서 가져온 정보를 볼 수 있습니다. 정보는 AEM Sites 페이지에 관련되어 있습니다. 기본적으로 이 정보는 Adobe Analytics에서 주기적으로 새로 고쳐지며 실시간으로 표시되지 않습니다.
...
>[!NOTE]
>
>AEM Forms Analytics 데이터는 오프라인에서 또는 Adobe Analytics Cloud 구성 없이 사용할 수 있습니다. 이 기능은 `we-gov-forms.ui.analytics-<version>.zip` 패키지가 설치된 경우 true입니다. 그러나 AEM Sites 데이터에는 활성 클라우드 구성이 필요합니다.
...
>[!NOTE]
>
>이 섹션에서 만든 인증서는 Adobe I/O에서 통합 서비스를 만드는 데 사용됩니다. 사용자가 통합 서비스를 만들면 사용자는 Adobe I/O에서 해당 정보를 사용하여 구성을 완료할 수 있습니다.
...
1. 올바른 그룹(오른쪽 상단 드롭다운 목록)에 있는지 확인하십시오.
...
1. AEM(**도구** > **보안** > **IMS**) 내에서 IMS 구성으로 이동합니다.

1. IMS 구성 화면에서 **다음**&#x200B;을(를) 클릭합니다.

1. 인증 서버(스크린샷에 표시된 값)를 입력합니다.

1. API 키를 입력합니다.

1. 클라이언트 암호를 입력하십시오(Adobe I/O 통합에서 **노출**&#x200B;을 클릭하여 표시).

1. Adobe I/O에서 JWT 탭을 클릭하여 JWT 페이로드를 가져오고 IMS 구성의 페이로드에 붙여넣습니다.

   ![페이로드 IMS 구성](assets/aftia-payload-ims-config.jpg)

1. 만든 후 IMS 구성을 클릭하고 상태 검사 를 선택하면 사용자에게 다음 결과가 표시됩니다.
...
1. 시스템의 서비스 URL이 `https://aemformsconversion.adobe.io/`(으)로 설정되어 있습니다.
...
1. 변환 프로세스가 시작되면 사용자에게 다음이 표시됩니다.

   ![전환 설정](assets/aftia-conversion-in-progress.jpg)

1. 전환이 완료되면 사용자에게 다음이 표시됩니다.
...
1. 템플릿 폴더에 모든 사용자 권한에 대해 `jcr:read`이(가) 활성화되어 있는지 확인하십시오. 권한이 설정되지 않은 경우 서비스 사용자가 저장소에서 템플릿을 읽을 수 없고 전환이 실패합니다.
...
이 패키지에 포함된 편집 가능한 템플릿에는 페이지 매김, 스타일 지정 등을 위해 Bootstrap 4 CSS 클래스를 사용하는 템플릿/페이지 정책으로 사전 구성된 템플릿도 포함되어 있습니다. 모든 클래스가 템플릿 정책에 추가되지는 않았지만 Bootstrap 4에서 지원하는 모든 클래스를 정책에 추가할 수 있습니다. 사용 가능한 클래스 목록은 다음을 참조하십시오.
...
지원되는 스타일에 대한 자세한 내용은 [템플릿 사용자 지정 스타일 시스템](../../forms/using/forms-install-configure-gov-reference-site.md#customizetemplates)을 참조하십시오.
...
특정 사용 사례에 맞게 이러한 양식을 사용자 지정할 수 있습니다. 특정 필드 및 제출 논리를 편집하여 양식이 다음과 같이 계속 제대로 작동하도록 하지 마십시오.

**의료 혜택에 대한 등록 응용 프로그램:**

* contact_id - 제출 중 MS® Dynamics 연락처 ID를 받는 데 사용되는 숨겨진 필드
* 제출 - 제출 단추 논리에는 콜백을 지원하기 위한 사용자 지정이 필요합니다. 이 설명서에서는 사용자 지정에 대해 설명합니다. 그러나 양식을 제출하고 Forms 데이터 모델을 통해 MS® Dynamics에 `POST` 및 `GET` 작업을 모두 수행하려면 큰 스크립트를 작성해야 합니다.
...
등록 적응형 Forms이 처리를 위해 OSGI 워크플로우에 제출됩니다. 이 워크플로는 *https://&lt;aemserver>:&lt;port>/conf/we-gov/settings/models/we-gov-process.html*&#x200B;에 있습니다.
...
* 등록 적응형 양식은 양식 이메일 필드에 지정된 이메일로 Adobe Sign 이메일을 전송합니다. 해당 이메일 주소는 Adobe Sign 클라우드 구성을 구성하는 데 사용되는 이메일과 동일한 이메일 주소가 될 수 없습니다.

### 알려진 문제 {#known-issues}

* (AGRS-120) 사이트 탐색 구성 요소는 현재 두 개 이상 깊이의 중첩된 하위 페이지를 지원하지 않습니다.
* (AGRS-159) 현재 MS® Dynamics FDM은 먼저 두 가지 작업을 수행하여 등록 적응형 양식 데이터를 Dynamics에 게시한 다음 사용자 레코드를 가져와 연락처 ID를 검색해야 합니다. 현재 상태에서 Dynamics에 이름이 같은 사용자가 두 명 이상 있어 등록 적응형 양식의 제출을 허용하지 않는 경우 연락처 ID를 가져올 수 없습니다.
...
페이지가 설치되면 Chrome 브라우저 내에서 테스트할 페이지를 로드합니다. 여러 탭이 열려 있으면 점수에 영향을 줄 수 있습니다. 이와 같이, 탭은 하나씩만 열려 있는 것이 바람직하다. 페이지가 로드되면 페이지에서 **마우스 오른쪽 단추를 클릭**&#x200B;하고 **감사** 탭을 선택합니다. 여기에서 개발자는 접근성 플러그인이 수행하는 감사 유형을 선택합니다. 원하는 옵션을 선택하면 사용자가 **보고서 생성**&#x200B;을 클릭하여 PDF 문서를 만듭니다. PDF은 전반적인 접근성 등급과 접근성 등급을 전체적으로 높이기 위해 사용할 수 있는 항목을 보여 줍니다.
...
이 데이터를 내보내려면 화면 오른쪽에 있는 세 개의 버튼을 클릭하고 사용 가능한 옵션 중에서 선택합니다.
...
`we-gov-forms.pkg.all-<version>.zip` 설치 가능한 ZIP 파일입니다. 이 패키지는 CRX을 사용하여 설치됩니다.

패키지 관리자 사용자는 **Forms** > **테마** > **참조 테마** > **Ultramarine에 액세스 가능**&#x200B;으로 이동하여 AEM Forms에서 Ultramarine 테마에 액세스할 수 있습니다.
...
We.Gov 참조 사이트를 탐색할 수 있습니다. We.Gov 참조 사이트 워크플로우 및 단계에 대한 자세한 내용은 [We.Gov 참조 사이트 연습](../../forms/using/forms-gov-reference-site-user-demo.md)을 참조하십시오.
