...
/libs 아래의 노드 및/또는 파일을 /apps 아래의 추가 노드와 오버레이하여 기본 제공 AEM 기능을 확장하는 것이 일반적인 방법입니다. 이러한 오버레이는 버전 제어에서 추적하고 대상 버전의 AEM에 대해 테스트해야 합니다. Adobe 파일(예: JS, JSP, HTL)이 오버레이되면 AEM의 대상 버전에서 보다 쉬운 회귀 테스트를 위해 보강된 기능에 주석을 남기는 것이 좋습니다. 일반 정보는 [오버레이](/help/sites-developing/overlays.md)를 참조하십시오. 특정 AEM 오버레이에 대한 지침은 아래에 나와 있습니다.
