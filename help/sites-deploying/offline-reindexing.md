---
title: AEM에 대한 오프라인 리인덱싱
description: 오프라인 리인덱싱 방법론을 사용하여 AEM 저장소를 다시 인덱싱하는 방법을 알아봅니다.
feature: Upgrading
...
일반적으로 큰 데이터 저장소와 높은 수준의 에셋 업로드가 있는 AEM Assets 프로젝트의 경우 Oak 색인을 다시 색인화하는 데 상당한 시간이 걸릴 수 있습니다.

이 섹션에서는 Oak 실행 도구를 사용하여 오프라인 리인덱싱을 수행하는 방법에 대해 설명합니다. 표시된 단계는 AEM 6.4 이상 버전의 [Lucene](https://jackrabbit.apache.org/oak/docs/query/lucene.html) 인덱스에 적용할 수 있습니다.

## 개요 {#overview}

AEM 저장소는 색인 정의 변경, 성능 최적화 등 다양한 이유로 인해 또는 중요한 콘텐츠 변경 후 다시 색인화해야 하는 경우가 많습니다. 리인덱싱은 에셋의 텍스트(예: PDF 파일의 텍스트)가 추출되고 인덱싱되므로 에셋 배포에 많은 비용이 소요됩니다. MongoMK 저장소를 사용하면 데이터가 네트워크를 통해 유지되므로 리인덱싱에 걸리는 시간이 더욱 늘어납니다. 해결 방법은 Oak 실행 도구를 사용하여 **offline** 리인덱싱을 수행한 다음 미리 빌드된 인덱스를 실행 중인 AEM 인스턴스로 가져오는 것입니다. 이 방법을 사용하면 리인덱싱 시간을 최소화하고 보다 효율적인 리소스 관리를 수행할 수 있습니다.

## 접근법 {#approach}

![오프라인 리인덱싱 업그레이드 텍스트 추출](assets/offline-reindexing-upgrade-process.png)

[Oak-run](/help/sites-deploying/indexing-via-the-oak-run-jar.md) 도구를 사용하여 인덱스를 오프라인으로 만든 다음 실행 중인 AEM 인스턴스로 가져오는 것이 좋습니다. 위의 다이어그램은 오프라인 리인덱싱 접근 방식을 보여 줍니다.

또한 접근 방식에 설명된 대로 단계 순서는 다음과 같습니다.

1. 바이너리의 텍스트가 먼저 추출됩니다.
2. 색인 정의가 생성되거나 업데이트됩니다
3. 오프라인 색인이 만들어집니다.
4. 그런 다음 실행 중인 AEM 인스턴스로 인덱스를 가져옵니다

### 텍스트 추출 {#text-extraction}

AEM에서 전체 인덱싱을 활성화하려면 PDF과 같은 바이너리의 텍스트가 추출되어 인덱스에 추가됩니다. 이는 일반적으로 인덱싱 프로세스의 많은 비용이 드는 단계입니다. 텍스트 추출은 자산 저장소가 많은 바이너리를 저장할 때 특히 리인덱싱하는 최적화 단계입니다.

![오프라인 리인덱싱 업그레이드 텍스트 추출](assets/offline-reindexing-upgrade-text-extraction.png)

시스템에 저장된 바이너리의 텍스트는 tika 라이브러리와 함께 oak-run 도구를 사용하여 추출할 수 있습니다. 이 텍스트 추출 프로세스에 운영 시스템의 클론을 가져와 사용할 수 있습니다. 그런 다음 다음 다음 단계를 수행하여 텍스트 저장소를 만듭니다.
...
>[!NOTE]
>
>AEM에서 사용되는 것과 동일한 버전의 Tika를 사용합니다.

여기서 `datastore path`은(는) 이진 데이터 저장소의 경로입니다.

생성된 텍스트 저장소는 향후 리인덱싱 시나리오에 업데이트하고 다시 사용할 수 있습니다.
...
오프라인으로 Lucene 인덱스를 만듭니다. MongoMK를 사용하는 경우 네트워크 오버헤드를 방지하므로 MongoMK 노드 중 하나에서 직접 실행하는 것이 좋습니다.

색인을 오프라인으로 만들려면 아래 단계를 수행하십시오.

**1. Oak Lucene 인덱스 정의 생성**

기존 인덱스 정의를 덤프합니다. Adobe Granite 저장소 번들 및 oak-run을 사용하여 인덱스 정의를 생성할 수 있습니다.

AEM 인스턴스에서 인덱스 정의를 덤프하려면 이 명령을 실행합니다.
...
여기서 `datastore path` 및 `nodestore path`은(는) AEM 인스턴스에서 가져온 것입니다.

그런 다음 적절한 Granite 저장소 번들을 사용하여 색인 정의를 생성합니다.
...
위의 단계에서는 인덱스 정의를 포함하는 `merge-index-definitions_target.json`이라는 JSON 파일을 만듭니다.
...
AEM 6.4 이상 버전에서는 AEM에 시작 시퀀스 중에 디스크에서 인덱스를 가져오는 기능이 내장되어 있습니다. 시작하는 동안 `<repository>/indexing-result/indexes` 폴더에서 인덱스 데이터가 있는지 확인합니다. AEM 인스턴스를 시작하기 전에 미리 만들어진 인덱스를 위의 위치에 복사할 수 있습니다. AEM은 이를 저장소로 가져오고 시스템에서 해당 체크포인트를 제거합니다. 따라서 색인 재지정은 완전히 방지됩니다.
...
프로덕션에서 리인덱싱 프로세스를 실행하기 전에 Runbook을 준비하고 몇 가지 시도를 수행하는 것이 좋습니다.
